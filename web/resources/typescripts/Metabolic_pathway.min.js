...
var __extends=this&&this.__extends||function(){var d=function(b,e){d=Object.setPrototypeOf||{__proto__:[]}instanceof Array&&function(c,a){c.__proto__=a}||function(c,a){for(var f in a)a.hasOwnProperty(f)&&(c[f]=a[f])};return d(b,e)};return function(b,e){function c(){this.constructor=b}d(b,e);b.prototype=null===e?Object.create(e):(c.prototype=e.prototype,new c)}}(),biodeep;
...
function(a){return c.d3cola.on("tick",c.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};b.prototype.loadGraph=function(c){var a=this,f=this.vis.append("g"),g=this.vis.append("g"),e=this.vis.append("g");console.log(JSON.stringify(c));this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",
5).attr("markerWidth",3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");f=f.selectAll(".group").data(c.groups).enter().append("rect").attr("rx",8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});e=e.selectAll(".link").data(c.links).enter().append("line").attr("class","link");var k=g.selectAll(".node").data(c.nodes).enter().append("rect").attr("class","node").attr("width",function(c){return c.width+
2*a.pad+2*a.margin}).attr("height",function(c){return c.height+2*a.pad+2*a.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);c=g.selectAll(".label").data(c.nodes).enter().append("text").attr("class","label").call(this.d3cola.drag);c.each(b.insertLinebreaks);k.append("title").text(function(a){return a.label});return new d.Cola_graph(k,e,f,c,this)};b.insertLinebreaks=function(c){var a=d3.select(this);c=c.label.split(" ");a.text("");for(var f=0;f<
c.length;f++){var b=c[f];b=a.append("tspan").text(b);b.attr("x",0).attr("dy","15").attr("font-size","12")}};return b}();d.Metabolic_pathway=b})(apps||(apps={}));
...
"flow"===a.itemType?go.Link.Normal:go.Link.Bezier;this.temporaryLink.path.stroke="flow"===a.itemType?"blue":"green";this.temporaryLink.path.strokeWidth="flow"===a.itemType?5:1;go.LinkingTool.prototype.doActivate.call(this)},"linkingTool.insertLink":function(b,f,d,e){c.myDiagram.model.setCategoryForLinkData(this.archetypeLinkData,a.itemType);this.archetypeLabelNodeData="flow"===a.itemType?{category:"valve"}:null;this.archetypeLinkData.text=a.itemType;return go.LinkingTool.prototype.insertLink.call(this,
b,f,d,e)},"clickCreatingTool.archetypeNodeData":{},"commandHandler.archetypeGroupData":{text:"Group",isGroup:!0,color:"blue"},"clickCreatingTool.isDoubleClick":!1,"clickCreatingTool.canStart":function(){return"node"===a.mode&&go.ClickCreatingTool.prototype.canStart.call(this)},"clickCreatingTool.insertPart":function(c){a.nodeCounter[a.itemType]+=1;var b="T"+a.itemType+a.nodeCounter[a.itemType];this.archetypeNodeData={key:b,category:a.itemType,label:b};return go.ClickCreatingTool.prototype.insertPart.call(this,
...
a.adornedPart;var c=a.memberParts.count,b=0;a.memberParts.each(function(a){a instanceof go.Link&&b++});return"Group "+a.data.key+": "+a.data.text+"\n"+c+" members including "+b+" links"};c.prototype.setMode=function(a,c){var b=this.myDiagram,f=this.SD;b.startTransaction();document.getElementById(f.itemType).className=f.mode+"_normal";document.getElementById(c).className=a+"_selected";f.mode=a;f.itemType=c;"pointer"===a?(b.allowLink=!1,b.nodes.each(function(a){return a.port.cursor=""})):"node"===a?
...
(function(d){var b=/[-]?\d+/ig;d.ModelPatch=function(d){for(var c=0,a=d.nodeDataArray;c<a.length;c++){var f=a[c];Strings.IsPattern(f.key.toString(),b)&&(f.key="T"+f.key)}c=0;for(a=d.linkDataArray;c<a.length;c++)if(f=a[c],Strings.IsPattern(f.from.toString(),b)&&(f.from="T"+f.from),Strings.IsPattern(f.to.toString(),b)&&(f.to="T"+f.to),!isNullOrEmpty(f.labelKeys))for(var e=0;e<f.labelKeys.length;e++)Strings.IsPattern(f.labelKeys[e].toString(),b)&&(f.labelKeys[e]="T"+f.labelKeys[e]);return d}})(apps||
(apps={}));
(function(d){(function(b){b.nodeStyle=function(){return[{type:go.Panel.Spot,layerName:"Background",locationObjectName:"SHAPE",selectionObjectName:"SHAPE",locationSpot:go.Spot.Center},(new go.Binding("location","loc",go.Point.parse)).makeTwoWay(go.Point.stringify)]};b.shapeStyle=function(){return{name:"SHAPE",stroke:"black",fill:"#f0f0f0",portId:"",fromLinkable:!0,toLinkable:!0}};b.textStyle=function(){return[{font:"bold 11pt helvetica, bold arial, sans-serif",margin:2,editable:!0},(new go.Binding("text","label")).makeTwoWay()]}})(d.EditorTemplates||
(d.EditorTemplates={}))})(apps||(apps={}));
...
var Graph=function(){function d(b){void 0===b&&(b=null);this.last_index=0;isNullOrUndefined(b)||(this.nodes=b.nodes,this.links=b.links)}d.prototype.objectify=function(){for(var b=this,d=b.links,c=[],a=function(a,f){var e=d[a];c.push(function(){var a;var c=b.nodes;var d=[];var f=0;for(a=c.length;f<a;f++){var g=c[f];e.source===g.id?e.source=g:e.target===g.id?e.target=g:d.push(void 0)}return d}())},f=0,g=d.length;f<g;f++)a(f,g);return c};d.prototype.remove=function(b){if(0<=__indexOf.call(this.nodes,
...
(function(d){var b=function(){function b(b){void 0===b&&(b="#embeddedsvg");this.nodeLookup={};this.sbsvg=d3.select($ts(b)).select("svg");this.graph={nodes:[],links:[],constraints:[],groups:[]}}b.prototype.getGraph=function(){var b=this;this.loadNodes(this.sbsvg);for(var a=0,d=this.sbsvg.selectAll(".connector")[0];a<d.length;a++){var e=d[a],h=e.getAttribute("dunnart:srcid");e=e.getAttribute("dunnart:dstid");h in this.nodeLookup?e in this.nodeLookup?this.graph.links.push({source:this.nodeLookup[h].index,
target:this.nodeLookup[e].index}):console.warn("missing node of "+e):console.warn("missing node of "+h)}a=0;for(d=this.sbsvg.selectAll(".cluster")[0];a<d.length;a++)h=d[a],this.graph.groups.push({leaves:h.getAttribute("dunnart:contains").split(" ").map(function(a){return b.nodeLookup[a].index}),style:h.getAttribute("style"),padding:10});a={};d=0;for(h=this.sbsvg.selectAll("[relType=alignment]")[0];d<h.length;d++){var k=h[d];e=k.getAttribute("constraintid");var l=this.nodeLookup[k.getAttribute("objoneid")].index;
k=parseInt(k.getAttribute("alignmentpos"));l={node:l,offset:0};e in a?a[e].offsets.push(l):this.graph.constraints.push(a[e]={type:"alignment",offsets:[l],axis:1==k?"y":"x"})}return this.graph};b.prototype.loadNodes=function(b){var a=0,c=0;for(b=b.selectAll("rect.shape")[0];c<b.length;c++){var d=b[c];this.graph.nodes.push(this.nodeLookup[d.id]={label:d.getAttribute("dunnart:label"),dunnartid:d.id,index:a++,width:60,height:40,x:parseFloat(d.getAttribute("x")),y:parseFloat(d.getAttribute("y")),rx:"rx"in
