...
(function(b){var d=function(){function d(){this.width=800;this.height=520;this.margin=6;this.pad=12;this.width=parseFloat($ts("@width"));this.height=parseFloat($ts("@height"))}d.prototype.redraw=function(){this.vis.attr("transform","translate("+d3.event.translate+") scale("+d3.event.scale+")")};d.prototype.savePng=function(){saveSvgAsPng($ts("#canvas").childNodes.item(0),"pathway.png")};d.prototype.init=function(){var c=this,a=$ts("@url:graph");this.d3cola=cola.d3adaptor(d3).linkDistance(60).avoidOverlaps(!0).size([this.width,
this.height]);this.outer=d3.select($ts("#canvas")).append("svg").attr("width",this.width).attr("height",this.height).attr("pointer-events","all");this.outer.append("rect").attr("class","background").attr("width","100%").attr("height","100%").call(d3.behavior.zoom().on("zoom",function(){return c.redraw()}));this.vis=this.outer.append("g").attr("transform","translate(80,80) scale(0.7)");if("#"==a.charAt(0))this.d3cola.on("tick",this.loadGraph(d.readGraph(a)).tick());else $ts.getText(a,function(a){return c.d3cola.on("tick",
c.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};d.readGraph=function(c){var a=$ts(c);return"script"==a.tagName.toLowerCase()?(c=JSON.parse(a.innerText),b.translation.translateToColaGraph(c)):(new dataAdapter.parseDunnart(c)).getGraph()};d.prototype.loadGraph=function(c){var a=this,g=this.vis.append("g"),f=this.vis.append("g"),e=this.vis.append("g");Strings.Empty(c.class)||"GraphLinksModel"!=c.class||(c=b.translation.translateToColaGraph(c));
console.log(JSON.stringify(c));console.log("view nodes details:");console.table(c.nodes);this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",5).attr("markerWidth",3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");g=g.selectAll(".group").data(c.groups).enter().append("rect").attr("rx",
8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});e=e.selectAll(".link").data(c.links).enter().append("line").attr("class","link");var h=f.selectAll(".node").data(c.nodes).enter().append("rect").attr("class","node").attr("width",function(c){return c.width+2*a.pad+2*a.margin}).attr("height",function(c){return c.height+2*a.pad+2*a.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);c=f.selectAll(".label").data(c.nodes).enter().append("text").attr("class",
"label").call(this.d3cola.drag);c.each(d.insertLinebreaks);h.append("title").text(function(a){return a.label});return new b.Cola_graph(h,e,g,c,this)};d.insertLinebreaks=function(c){var a=d3.select(this);c=c.label.split(" ");a.text("");for(var b=0;b<c.length;b++){var f=c[b];f=a.append("tspan").text(f);f.attr("x",0).attr("dy","15").attr("font-size","12")}};return d}();b.Metabolic_pathway=d})(apps||(apps={}));
(function(b){(function(b){b.paper_colors="#D02823 #0491d0 #88bb64 #15DBFF #583B73 #f2ce3f #8858BF #CCFF33 #fb5b44 #361f32 #DF2789 #396b1c".split(" ");b.translateToColaGraph=function(d){for(var c={groups:[],constraints:[],links:[],nodes:[]},a={},g={},f=0,e=d.nodeDataArray;f<e.length;f++){var h=e[f];if(!isNullOrUndefined(h.isGroup)&&h.isGroup){var k=b.paper_colors[Object.keys(g).length];g[h.key.toString()]={leaves:[],padding:10,style:"fill:"+k+";fill-opacity:0.31764700000000001;stroke:"+k+";stroke-opacity:1"}}}f=
0;for(e=d.nodeDataArray;f<e.length;f++)h=e[f],!isNullOrUndefined(h.isGroup)&&h.isGroup||"valve"==h.category||(k=h.loc,k=Strings.Empty(k)?[0,0]:k.split(/\s+/ig).map(function(a){return parseFloat(a)}),2>k.length&&(k=[0,0]),c.nodes.push({dunnartid:(c.nodes.length+1).toString(),height:40,index:c.nodes.length,label:h.label,rx:9,ry:9,width:60,x:k[0],y:k[1]}),a[h.key]=c.nodes.length-1,isNullOrUndefined(h.group)||g[h.group.toString()].leaves.push(c.nodes.length-1));h=0;for(d=d.linkDataArray;h<d.length;h++)f=
d[h],c.links.push({source:a[f.from],target:a[f.to]});for(var m in g)c.groups.push(g[m]);return c}})(b.translation||(b.translation={}))})(apps||(apps={}));
...
(function(b){var d=function(d){function c(){var a=null!==d&&d.apply(this,arguments)||this;a.assemblyKey="ko00001-assembly";a.canvas=new b.Metabolic_pathway;return a}__extends(c,d);Object.defineProperty(c.prototype,"appName",{get:function(){return"Pathway_explorer"},enumerable:!0,configurable:!0});c.prototype.init=function(){var a=$ts("@data:repository"),c=this,b=localStorage.getItem(this.assemblyKey);Strings.Empty(a,!0)||(Strings.Empty(b)?$ts.get(a,function(a){c.saveCache(a);c.loadCache()}):c.loadCache());
c.canvas.init()};c.prototype.loadUITree=function(a){a=PathwayNavigator.parseJsTree(a);var b=$ts("@app:explorer");$("#"+b).jstree({core:{data:a.children},plugins:["contextmenu"],contextmenu:{items:{add_reactor:{label:"Add Reactor",action:c.addReactor}}}})};c.addReactor=function(a){a.reference[0].id.match(/K\d{4,}/g)};c.prototype.saveCache=function(a){var c=[],b=0;for(a=a.children;b<a.length;b++){var d=a[b],e=d.name.toLowerCase().replace(/\s+/ig,"_");c.push(e);localStorage.setItem(e,JSON.stringify(d))}localStorage.setItem(this.assemblyKey,
JSON.stringify(c))};c.prototype.loadCache=function(){var a=localStorage.getItem(this.assemblyKey),c={name:"ko00001",children:[]},b=0;for(a=JSON.parse(a);b<a.length;b++){var d=a[b];TypeScript.logging.log("load cache: "+d+"...");d=localStorage.getItem(d);c.children.push(JSON.parse(d))}this.loadUITree(c)};return c}(Bootstrap);b.PathwayExplorer=d})(apps||(apps={}));var PathwayNavigator;
...
target:this.nodeLookup[f].index}):console.warn("missing node of "+f):console.warn("missing node of "+e)}a=0;for(d=this.sbsvg.selectAll(".cluster")[0];a<d.length;a++)e=d[a],this.graph.groups.push({leaves:e.getAttribute("dunnart:contains").split(" ").map(function(a){return b.nodeLookup[a].index}),style:e.getAttribute("style"),padding:10});a={};d=0;for(e=this.sbsvg.selectAll("[relType=alignment]")[0];d<e.length;d++){var h=e[d];f=h.getAttribute("constraintid");var k=this.nodeLookup[h.getAttribute("objoneid")].index;
h=parseInt(h.getAttribute("alignmentpos"));k={node:k,offset:0};f in a?a[f].offsets.push(k):this.graph.constraints.push(a[f]={type:"alignment",offsets:[k],axis:1==h?"y":"x"})}return this.graph};b.prototype.loadNodes=function(b){var a=0,c=0;for(b=b.selectAll("rect.shape")[0];c<b.length;c++){var d=b[c];this.graph.nodes.push(this.nodeLookup[d.id]={label:d.getAttribute("dunnart:label"),dunnartid:d.id,index:a++,width:60,height:40,x:parseFloat(d.getAttribute("x")),y:parseFloat(d.getAttribute("y")),rx:"rx"in
