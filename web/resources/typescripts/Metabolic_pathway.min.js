var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.SIMPLE_FROUND_POLYFILL=!1;$jscomp.defineProperty=$jscomp.ASSUME_ES5||"function"==typeof Object.defineProperties?Object.defineProperty:function(d,b,e){d!=Array.prototype&&d!=Object.prototype&&(d[b]=e.value)};$jscomp.getGlobal=function(d){return"undefined"!=typeof window&&window===d?d:"undefined"!=typeof global&&null!=global?global:d};$jscomp.global=$jscomp.getGlobal(this);
$jscomp.polyfill=function(d,b,e,c){if(b){e=$jscomp.global;d=d.split(".");for(c=0;c<d.length-1;c++){var a=d[c];a in e||(e[a]={});e=e[a]}d=d[d.length-1];c=e[d];b=b(c);b!=c&&null!=b&&$jscomp.defineProperty(e,d,{configurable:!0,writable:!0,value:b})}};$jscomp.underscoreProtoCanBeSet=function(){var d={a:!0},b={};try{return b.__proto__=d,b.a}catch(e){}return!1};
$jscomp.setPrototypeOf="function"==typeof Object.setPrototypeOf?Object.setPrototypeOf:$jscomp.underscoreProtoCanBeSet()?function(d,b){d.__proto__=b;if(d.__proto__!==b)throw new TypeError(d+" is not extensible");return d}:null;$jscomp.polyfill("Object.setPrototypeOf",function(d){return d||$jscomp.setPrototypeOf},"es6","es5");
var __extends=this&&this.__extends||function(){var d=function(b,e){d=Object.setPrototypeOf||{__proto__:[]}instanceof Array&&function(c,a){c.__proto__=a}||function(c,a){for(var f in a)a.hasOwnProperty(f)&&(c[f]=a[f])};return d(b,e)};return function(b,e){function c(){this.constructor=b}d(b,e);b.prototype=null===e?Object.create(e):(c.prototype=e.prototype,new c)}}(),biodeep;
(function(d){(function(b){b.start=function(){Router.AddAppHandler(new apps.PathwayExplorer);Router.RunApp()}})(d.app||(d.app={}))})(biodeep||(biodeep={}));var __indexOf=Array.prototype.indexOf||function(d){for(var b=0,e=this.length;b<e;b++)if(b in this&&this[b]===d)return b;return-1};$ts.mode=Modes.debug;$ts(biodeep.app.start);var apps;
(function(d){var b=function(){function b(c,a,f,b,e){this.node=c;this.link=a;this.group=f;this.label=b;this.vm=e;console.log(this)}b.prototype.tick=function(){var c=this.vm,a=this;return function(){a.node.each(function(a){return a.innerBounds=a.bounds.inflate(-c.margin)});a.link.each(function(a){return a.route=cola.makeEdgeBetween(a.source.innerBounds,a.target.innerBounds,5)});a.link.attr("x1",function(a){return a.route.sourceIntersection.x}).attr("y1",function(a){return a.route.sourceIntersection.y}).attr("x2",
function(a){return a.route.arrowStart.x}).attr("y2",function(a){return a.route.arrowStart.y});a.label.each(function(a){var f=this.getBBox();a.width=f.width+2*c.margin+8;a.height=f.height+2*c.margin+8});a.node.attr("x",function(a){return a.innerBounds.x}).attr("y",function(a){return a.innerBounds.y}).attr("width",function(a){return a.innerBounds.width()}).attr("height",function(a){return a.innerBounds.height()});a.group.attr("x",function(a){return a.bounds.x}).attr("y",function(a){return a.bounds.y}).attr("width",
function(a){return a.bounds.width()}).attr("height",function(a){return a.bounds.height()});a.label.attr("transform",function(c){return a.transform(c)})}};b.prototype.transform=function(c){return"translate("+c.x+this.vm.margin+", "+(c.y+this.vm.margin-c.height/2)+")"};return b}();d.Cola_graph=b})(apps||(apps={}));
(function(d){var b=function(){function b(){this.width=800;this.height=520;this.margin=6;this.pad=12;this.width=parseFloat($ts("@width"));this.height=parseFloat($ts("@height"))}b.prototype.redraw=function(){this.vis.attr("transform","translate("+d3.event.translate+") scale("+d3.event.scale+")")};b.prototype.savePng=function(){saveSvgAsPng($ts("#canvas").childNodes.item(0),"pathway.png")};b.prototype.init=function(){var c=this,a=$ts("@url:graph");this.d3cola=cola.d3adaptor(d3).linkDistance(60).avoidOverlaps(!0).size([this.width,
this.height]);this.outer=d3.select($ts("#canvas")).append("svg").attr("width",this.width).attr("height",this.height).attr("pointer-events","all");this.outer.append("rect").attr("class","background").attr("width","100%").attr("height","100%").call(d3.behavior.zoom().on("zoom",function(){return c.redraw()}));this.vis=this.outer.append("g").attr("transform","translate(80,80) scale(0.7)");if("#"==a.charAt(0))this.d3cola.on("tick",this.loadGraph((new dataAdapter.parseDunnart(a)).getGraph()).tick());else $ts.getText(a,
function(a){return c.d3cola.on("tick",c.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};b.prototype.loadGraph=function(c){var a=this,f=this.vis.append("g"),g=this.vis.append("g"),e=this.vis.append("g");console.log(JSON.stringify(c));this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",
5).attr("markerWidth",3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");f=f.selectAll(".group").data(c.groups).enter().append("rect").attr("rx",8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});e=e.selectAll(".link").data(c.links).enter().append("line").attr("class","link");var h=g.selectAll(".node").data(c.nodes).enter().append("rect").attr("class","node").attr("width",function(c){return c.width+
2*a.pad+2*a.margin}).attr("height",function(c){return c.height+2*a.pad+2*a.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);c=g.selectAll(".label").data(c.nodes).enter().append("text").attr("class","label").call(this.d3cola.drag);c.each(b.insertLinebreaks);h.append("title").text(function(a){return a.label});return new d.Cola_graph(h,e,f,c,this)};b.insertLinebreaks=function(c){var a=d3.select(this);c=c.label.split(" ");a.text("");for(var b=0;b<
c.length;b++){var g=c[b];g=a.append("tspan").text(g);g.attr("x",0).attr("dy","15").attr("font-size","12")}};return b}();d.Metabolic_pathway=b})(apps||(apps={}));
(function(d){var b=function(){function b(c){this.graph=c;this.width=960;this.height=500;this.drag_link=this.drag=this.new_link_source=this.force=this.colorify=this.vis=this.selection=null;c instanceof Graph||(this.graph=new Graph(c))}b.prototype.tick=function(){this.vis.selectAll(".node").attr("transform",function(c){return"translate("+c.x+","+c.y+")"});return this.vis.selectAll(".link").attr("x1",function(c){return c.source.x}).attr("y1",function(c){return c.source.y}).attr("x2",function(c){return c.target.x}).attr("y2",
function(c){return c.target.y})};b.prototype.click=function(c){this.selection=null;d3.selectAll(".node").classed("selected",!1);return d3.selectAll(".link").classed("selected",!1)};b.prototype.zoom=function(){return this.vis.attr("transform","translate("+d3.event.translate+")scale("+d3.event.scale+")")};b.prototype.init=function(){var c=this,a=d3.select("body").append("svg").attr("width",this.width).attr("height",this.height).append("g"),b=this;this.graph.objectify();a.call(d3.behavior.zoom().scaleExtent([.5,
8]).on("zoom",function(){return b.zoom()}));b.vis=a.append("g");b.vis.append("rect").attr("class","overlay").attr("x",-5E5).attr("y",-5E5).attr("width",1E6).attr("height",1E6).on("click",function(a){return c.click(a)});b.colorify=d3.scale.category10();b.force=d3.layout.force().size([this.width,this.height]).charge(-400).linkDistance(60).on("tick",function(){return b.tick()});b.drag=b.force.drag().on("dragstart",function(a){return a.fixed=!0});d3.select(window).on("keydown",function(){if(46===d3.event.keyCode&&
null!=b.selection)return b.graph.remove(b.selection),b.selection=null,b.update()});this.update();this.toolbar()};b.prototype.toolbar=function(){var c=$("<div class='toolbar'></div>"),a=this,b=$("<div class='library'></div></div>");$("body").append(c);c.append($(d.pointer));c.append($(d.add_node));c.append($(d.add_link));c.append(b);["X","Y","Z","W"].forEach(function(c){var g=$(d.buttonHtml(a,c));g.bind("click",function(){a.graph.add_node(c);return a.update()});b.append(g);return b.hide()});a.tool=
"pointer";a.new_link_source=null;a.vis.on("mousemove.add_link",function(c){if(null!=a.new_link_source)return c=d3.mouse(a.vis.node()),a.drag_link.attr("x1",a.new_link_source.x).attr("y1",a.new_link_source.y).attr("x2",c[0]).attr("y2",c[1])}).on("mouseup.add_link",function(c){a.new_link_source=null;if(null!=a.drag_link)return a.drag_link.remove()});return d3.selectAll(".tool").on("click",function(){return a.tool_click(b)})};b.prototype.tool_click=function(c){d3.selectAll(".tool").classed("active",
!1);d3.select(c).classed("active",!0);var a=$(c).data("tool"),b=this.vis.selectAll(".node");"add_link"===a&&"add_link"!==this.tool?(b.on("mousedown.drag",null).on("touchstart.drag",null),b.call(this.drag_add_link)):"add_link"!==a&&"add_link"===this.tool&&(b.on("mousedown.add_link",null).on("mouseup.add_link",null),b.call(this.drag));"add_node"===a?c.show():c.hide();return this.tool=a};b.prototype.node_click=function(c){this.selection=c;d3.selectAll(".node").classed("selected",function(a){return a===
c});return d3.selectAll(".link").classed("selected",!1)};b.prototype.link_click=function(c){this.selection=c;d3.selectAll(".link").classed("selected",function(a){return a===c});return d3.selectAll(".node").classed("selected",!1)};b.prototype.update=function(){var c=this.graph,a=this;a.force.nodes(c.nodes).links(c.links).start();var b=a.vis.selectAll(".node").data(c.nodes,function(a){return a.id}),d=b.enter().append("g").attr("class","node").on("click",function(c){return a.node_click(c)});c=a.vis.selectAll(".link").data(c.links,
function(a){return""+a.source.id+"->"+a.target.id});c.enter().insert("line",".node").attr("class","link").on("click",function(c){return a.link_click(c)});c.exit().remove();"add_link"===a.tool?d.call(this.drag_add_link):d.call(a.drag);d.append("circle").attr("r",18).attr("stroke",function(c){return a.colorify(c.type)}).attr("fill",function(c){return d3.hcl(a.colorify(c.type)).brighter(3)});d.append("text").text(function(a){return a.id}).attr("dy","0.35em").attr("fill",function(c){return a.colorify(c.type)});
return b.exit().remove()};b.prototype.drag_add_link=function(c){var a=this,b=this.graph;return c.on("mousedown.add_link",function(c){a.new_link_source=c;var b=d3.mouse(a.vis.node());a.drag_link=a.vis.insert("line",".node").attr("class","drag_link").attr("x1",c.x).attr("y1",c.y).attr("x2",b[0]).attr("y2",b[1]);d3.event.stopPropagation();return d3.event.preventDefault()}).on("mouseup.add_link",function(c){if(null!=b.add_link(a.new_link_source,c))return a.update()})};return b}();d.GraphEditor=b})(apps||
(apps={}));
(function(d){d.buttonHtml=function(b,d){return"\n            <svg width='42' height='42'>\n                <g class='node'>\n                    <circle cx = '21' cy = '21' r = '18'\n                        stroke = '"+b.colorify(d)+"'\n                        fill = '"+d3.hcl(b.colorify(d)).brighter(3)+"'\n                    />\n                </g>\n            </svg>\n        "};d.add_link="\n        <svg class='tool' data-tool='add_link' xmlns='http://www.w3.org/2000/svg' version='1.1' width='32' height='32' viewBox='0 0 128 128'>\n            <g transform='translate(557.53125,-356.22543)'>\n                <g transform='translate(20,0)'>\n                    <path d='m -480.84375,360 c -15.02602,0 -27.375,12.31773 -27.375,27.34375 0,4.24084 1.00221,8.28018 2.75,11.875 l -28.875,28.875 c -3.59505,-1.74807 -7.6338,-2.75 -11.875,-2.75 -15.02602,0 -27.34375,12.34898 -27.34375,27.375 0,15.02602 12.31773,27.34375 27.34375,27.34375 15.02602,0 27.375,-12.31773 27.375,-27.34375 0,-4.26067 -0.98685,-8.29868 -2.75,-11.90625 L -492.75,411.96875 c 3.60156,1.75589 7.65494,2.75 11.90625,2.75 15.02602,0 27.34375,-12.34898 27.34375,-27.375 C -453.5,372.31773 -465.81773,360 -480.84375,360 z m 0,14 c 7.45986,0 13.34375,5.88389 13.34375,13.34375 0,7.45986 -5.88389,13.375 -13.34375,13.375 -7.45986,0 -13.375,-5.91514 -13.375,-13.375 0,-7.45986 5.91514,-13.34375 13.375,-13.34375 z m -65.375,65.34375 c 7.45986,0 13.34375,5.91514 13.34375,13.375 0,7.45986 -5.88389,13.34375 -13.34375,13.34375 -7.45986,0 -13.34375,-5.88389 -13.34375,-13.34375 0,-7.45986 5.88389,-13.375 13.34375,-13.375 z'/>\n                    <path d='m -484.34375,429.25 c -1.95543,0.19978 -3.60373,2.03442 -3.59375,4 l 0,12.40625 -12.40625,0 c -2.09434,2.1e-4 -3.99979,1.90566 -4,4 l 0,10 c -0.007,0.1353 -0.007,0.27095 0,0.40625 0.19978,1.95543 2.03442,3.60373 4,3.59375 l 12.40625,0 0,12.4375 c 2.1e-4,2.09434 1.90566,3.99979 4,4 l 10,0 c 2.09434,-2.1e-4 3.99979,-1.90566 4,-4 l 0,-12.4375 12.4375,0 c 2.09434,-2.1e-4 3.99979,-1.90566 4,-4 l 0,-10 c -2.1e-4,-2.09434 -1.90566,-3.99979 -4,-4 l -12.4375,0 0,-12.40625 c -2.1e-4,-2.09434 -1.90566,-3.99979 -4,-4 l -10,0 c -0.1353,-0.007 -0.27095,-0.007 -0.40625,0 z'/>\n                </g>\n            </g>\n        </svg>\n    ";d.add_node=
"\n        <svg class='tool' data-tool='add_node' xmlns='http://www.w3.org/2000/svg' version='1.1' width='32' height='32' viewBox='0 0 128 128'>\n            <g transform='translate(720.71649,-356.22543)'>\n                <g transform='translate(-3.8571429,146.42857)'>\n                    <path d='m -658.27638,248.37149 c -1.95543,0.19978 -3.60373,2.03442 -3.59375,4 l 0,12.40625 -12.40625,0 c -2.09434,2.1e-4 -3.99979,1.90566 -4,4 l 0,10 c -0.007,0.1353 -0.007,0.27095 0,0.40625 0.19978,1.95543 2.03442,3.60373 4,3.59375 l 12.40625,0 0,12.4375 c 2.1e-4,2.09434 1.90566,3.99979 4,4 l 10,0 c 2.09434,-2.1e-4 3.99979,-1.90566 4,-4 l 0,-12.4375 12.4375,0 c 2.09434,-2.1e-4 3.99979,-1.90566 4,-4 l 0,-10 c -2.1e-4,-2.09434 -1.90566,-3.99979 -4,-4 l -12.4375,0 0,-12.40625 c -2.1e-4,-2.09434 -1.90566,-3.99979 -4,-4 l -10,0 c -0.1353,-0.007 -0.27095,-0.007 -0.40625,0 z'/>\n                    <path d='m -652.84375,213.9375 c -32.97528,0 -59.875,26.86847 -59.875,59.84375 0,32.97528 26.89972,59.875 59.875,59.875 32.97528,0 59.84375,-26.89972 59.84375,-59.875 0,-32.97528 -26.86847,-59.84375 -59.84375,-59.84375 z m 0,14 c 25.40911,0 45.84375,20.43464 45.84375,45.84375 0,25.40911 -20.43464,45.875 -45.84375,45.875 -25.40911,0 -45.875,-20.46589 -45.875,-45.875 0,-25.40911 20.46589,-45.84375 45.875,-45.84375 z'/>\n                </g>\n            </g>\n        </svg>\n    ";
d.pointer="\n        <svg class='active tool' data-tool='pointer' xmlns='http://www.w3.org/2000/svg' version='1.1' width='32' height='32' viewBox='0 0 128 128'>\n            <g transform='translate(881.10358,-356.22543)'>\n                <g transform='matrix(0.8660254,-0.5,0.5,0.8660254,-266.51112,-215.31898)'>\n                    <path d='m -797.14902,212.29589 a 5.6610848,8.6573169 0 0 0 -4.61823,4.3125 l -28.3428,75.0625 a 5.6610848,8.6573169 0 0 0 4.90431,13 l 56.68561,0 a 5.6610848,8.6573169 0 0 0 4.9043,-13 l -28.3428,-75.0625 a 5.6610848,8.6573169 0 0 0 -5.19039,-4.3125 z m 0.28608,25.96875 18.53419,49.09375 -37.06838,0 18.53419,-49.09375 z'/>\n                    <path d='m -801.84375,290.40625 c -2.09434,2.1e-4 -3.99979,1.90566 -4,4 l 0,35.25 c 2.1e-4,2.09434 1.90566,3.99979 4,4 l 10,0 c 2.09434,-2.1e-4 3.99979,-1.90566 4,-4 l 0,-35.25 c -2.1e-4,-2.09434 -1.90566,-3.99979 -4,-4 z'/>\n                </g>\n            </g>\n        </svg>\n    "})(apps||
(apps={}));
(function(d){var b=function(b){function c(){var a=null!==b&&b.apply(this,arguments)||this;a.assemblyKey="ko00001-assembly";a.canvas=new d.Metabolic_pathway;return a}__extends(c,b);Object.defineProperty(c.prototype,"appName",{get:function(){return"Pathway_explorer"},enumerable:!0,configurable:!0});c.prototype.init=function(){var a=$ts("@data:repository"),c=this,b=localStorage.getItem(this.assemblyKey);Strings.Empty(b)?$ts.get(a,function(a){c.saveCache(a);c.loadCache()}):c.loadCache();c.canvas.init()};c.prototype.loadUITree=
function(a){a=PathwayNavigator.parseJsTree(a);var b=$ts("@app:explorer");$("#"+b).jstree({core:{data:a.children},plugins:["contextmenu"],contextmenu:{items:{add_reactor:{label:"Add Reactor",action:c.addReactor}}}})};c.addReactor=function(a){a.reference[0].id.match(/K\d{4,}/g)};c.prototype.saveCache=function(a){var c=[],b=0;for(a=a.children;b<a.length;b++){var d=a[b],e=d.name.toLowerCase().replace(/\s+/ig,"_");c.push(e);localStorage.setItem(e,JSON.stringify(d))}localStorage.setItem(this.assemblyKey,
JSON.stringify(c))};c.prototype.loadCache=function(){var a=localStorage.getItem(this.assemblyKey),c={name:"ko00001",children:[]},b=0;for(a=JSON.parse(a);b<a.length;b++){var d=a[b];TypeScript.logging.log("load cache: "+d+"...");d=localStorage.getItem(d);c.children.push(JSON.parse(d))}this.loadUITree(c)};return c}(Bootstrap);d.PathwayExplorer=b})(apps||(apps={}));var PathwayNavigator;
(function(d){function b(c){var a=KEGG.brite.parseIDEntry(c.name),d=a.commonName||"KEGG";return isNullOrEmpty(c.children)?{id:a.id+"_"+ ++e,text:c.name,children:null,icon:"data:image/png;base64,"}:
{id:a.id+"_"+ ++e,text:d,children:$from(c.children).Select(b).ToArray(),icon:"data:image/png;base64,"}}
var e=1;d.parseJsTree=b})(PathwayNavigator||(PathwayNavigator={}));
var Graph=function(){function d(b){void 0===b&&(b=null);this.last_index=0;isNullOrUndefined(b)||(this.nodes=b.nodes,this.links=b.links)}d.prototype.objectify=function(){for(var b=this,d=b.links,c=[],a=function(a,e){var g=d[a];c.push(function(){var a;var c=b.nodes;var d=[];var e=0;for(a=c.length;e<a;e++){var f=c[e];g.source===f.id?g.source=f:g.target===f.id?g.target=f:d.push(void 0)}return d}())},f=0,g=d.length;f<g;f++)a(f,g);return c};d.prototype.remove=function(b){if(0<=__indexOf.call(this.nodes,
b))return this.nodes=this.nodes.filter(function(d){return d!==b}),this.links=this.links.filter(function(d){return d.source.id!==b.id&&d.target.id!==b.id});if(0<=__indexOf.call(this.links,b))return this.links=this.links.filter(function(d){return d!==b})};d.prototype.add_node=function(b){b={dunnartid:(this.last_index++).toString(),x:0,y:0,type:b};this.nodes.push(b);return b};d.prototype.add_link=function(b,d){if(b===d)return null;for(var c=this.links,a=0,e=c.length;a<e;a++){var g=c[a];if(g.source===
b&&g.target===d)return null}b={source:b,target:d};this.links.push(b);return b};return d}(),dataAdapter;
(function(d){var b=function(){function b(c){void 0===c&&(c="#embeddedsvg");this.nodeLookup={};this.sbsvg=d3.select($ts(c)).select("svg");this.graph={nodes:[],links:[],constraints:[],groups:[]}}b.prototype.getGraph=function(){var c=this;this.loadNodes(this.sbsvg);for(var a=0,b=this.sbsvg.selectAll(".connector")[0];a<b.length;a++){var d=b[a],e=d.getAttribute("dunnart:srcid");d=d.getAttribute("dunnart:dstid");e in this.nodeLookup?d in this.nodeLookup?this.graph.links.push({source:this.nodeLookup[e].index,
target:this.nodeLookup[d].index}):console.warn("missing node of "+d):console.warn("missing node of "+e)}a=0;for(b=this.sbsvg.selectAll(".cluster")[0];a<b.length;a++)e=b[a],this.graph.groups.push({leaves:e.getAttribute("dunnart:contains").split(" ").map(function(a){return c.nodeLookup[a].index}),style:e.getAttribute("style"),padding:10});a={};b=0;for(e=this.sbsvg.selectAll("[relType=alignment]")[0];b<e.length;b++){var h=e[b];d=h.getAttribute("constraintid");var k=this.nodeLookup[h.getAttribute("objoneid")].index;
h=parseInt(h.getAttribute("alignmentpos"));k={node:k,offset:0};d in a?a[d].offsets.push(k):this.graph.constraints.push(a[d]={type:"alignment",offsets:[k],axis:1==h?"y":"x"})}return this.graph};b.prototype.loadNodes=function(c){var a=0,b=0;for(c=c.selectAll("rect.shape")[0];b<c.length;b++){var d=c[b];this.graph.nodes.push(this.nodeLookup[d.id]={label:d.getAttribute("dunnart:label"),dunnartid:d.id,index:a++,width:60,height:40,x:parseFloat(d.getAttribute("x")),y:parseFloat(d.getAttribute("y")),rx:"rx"in
d.attributes?parseFloat(d.getAttribute("rx")):5,ry:"ry"in d.attributes?parseFloat(d.getAttribute("ry")):5})}};return b}();d.parseDunnart=b})(dataAdapter||(dataAdapter={}));
