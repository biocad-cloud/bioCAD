var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.SIMPLE_FROUND_POLYFILL=!1;$jscomp.defineProperty=$jscomp.ASSUME_ES5||"function"==typeof Object.defineProperties?Object.defineProperty:function(b,f,e){b!=Array.prototype&&b!=Object.prototype&&(b[f]=e.value)};$jscomp.getGlobal=function(b){return"undefined"!=typeof window&&window===b?b:"undefined"!=typeof global&&null!=global?global:b};$jscomp.global=$jscomp.getGlobal(this);
$jscomp.polyfill=function(b,f,e,c){if(f){e=$jscomp.global;b=b.split(".");for(c=0;c<b.length-1;c++){var d=b[c];d in e||(e[d]={});e=e[d]}b=b[b.length-1];c=e[b];f=f(c);f!=c&&null!=f&&$jscomp.defineProperty(e,b,{configurable:!0,writable:!0,value:f})}};$jscomp.underscoreProtoCanBeSet=function(){var b={a:!0},f={};try{return f.__proto__=b,f.a}catch(e){}return!1};
$jscomp.setPrototypeOf="function"==typeof Object.setPrototypeOf?Object.setPrototypeOf:$jscomp.underscoreProtoCanBeSet()?function(b,f){b.__proto__=f;if(b.__proto__!==f)throw new TypeError(b+" is not extensible");return b}:null;$jscomp.polyfill("Object.setPrototypeOf",function(b){return b||$jscomp.setPrototypeOf},"es6","es5");
var __extends=this&&this.__extends||function(){var b=function(f,e){b=Object.setPrototypeOf||{__proto__:[]}instanceof Array&&function(c,d){c.__proto__=d}||function(c,d){for(var a in d)d.hasOwnProperty(a)&&(c[a]=d[a])};return b(f,e)};return function(f,e){function c(){this.constructor=f}b(f,e);f.prototype=null===e?Object.create(e):(c.prototype=e.prototype,new c)}}(),biodeep;
(function(b){(function(b){b.start=function(){Router.AddAppHandler(new apps.PathwayExplorer);Router.RunApp()}})(b.app||(b.app={}))})(biodeep||(biodeep={}));$ts.mode=Modes.debug;$ts(biodeep.app.start);var apps;
(function(b){var f=function(){function b(c,d,a,b,e){this.node=c;this.link=d;this.group=a;this.label=b;this.vm=e;console.log(this)}b.prototype.tick=function(){var c=this.vm,d=this;return function(){d.node.each(function(a){return a.innerBounds=a.bounds.inflate(-c.margin)});d.link.each(function(a){return a.route=cola.makeEdgeBetween(a.source.innerBounds,a.target.innerBounds,5)});d.link.attr("x1",function(a){return a.route.sourceIntersection.x}).attr("y1",function(a){return a.route.sourceIntersection.y}).attr("x2",
function(a){return a.route.arrowStart.x}).attr("y2",function(a){return a.route.arrowStart.y});d.label.each(function(a){var d=this.getBBox();a.width=d.width+2*c.margin+8;a.height=d.height+2*c.margin+8});d.node.attr("x",function(a){return a.innerBounds.x}).attr("y",function(a){return a.innerBounds.y}).attr("width",function(a){return a.innerBounds.width()}).attr("height",function(a){return a.innerBounds.height()});d.group.attr("x",function(a){return a.bounds.x}).attr("y",function(a){return a.bounds.y}).attr("width",
function(a){return a.bounds.width()}).attr("height",function(a){return a.bounds.height()});d.label.attr("transform",function(a){return d.transform(a)})}};b.prototype.transform=function(c){return"translate("+c.x+this.vm.margin+", "+(c.y+this.vm.margin-c.height/2)+")"};return b}();b.Cola_graph=f})(apps||(apps={}));
(function(b){var f=function(){function e(){this.width=800;this.height=520;this.margin=6;this.pad=12;this.width=parseFloat($ts("@width"));this.height=parseFloat($ts("@height"))}e.prototype.redraw=function(){this.vis.attr("transform","translate("+d3.event.translate+") scale("+d3.event.scale+")")};e.prototype.savePng=function(){saveSvgAsPng($ts("#canvas").childNodes.item(0),"pathway.png")};e.prototype.init=function(){var c=this,d=$ts("@url:graph");this.d3cola=cola.d3adaptor(d3).linkDistance(60).avoidOverlaps(!0).size([this.width,
this.height]);this.outer=d3.select($ts("#canvas")).append("svg").attr("width",this.width).attr("height",this.height).attr("pointer-events","all");this.outer.append("rect").attr("class","background").attr("width","100%").attr("height","100%").call(d3.behavior.zoom().on("zoom",function(){return c.redraw()}));this.vis=this.outer.append("g").attr("transform","translate(80,80) scale(0.7)");if("#"==d.charAt(0))this.d3cola.on("tick",this.loadGraph((new dataAdapter.parseDunnart(d)).getGraph()).tick());else $ts.getText(d,
function(a){return c.d3cola.on("tick",c.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};e.prototype.loadGraph=function(c){var d=this,a=this.vis.append("g"),h=this.vis.append("g"),f=this.vis.append("g");console.log(JSON.stringify(c));this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",
5).attr("markerWidth",3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");a=a.selectAll(".group").data(c.groups).enter().append("rect").attr("rx",8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});f=f.selectAll(".link").data(c.links).enter().append("line").attr("class","link");var k=h.selectAll(".node").data(c.nodes).enter().append("rect").attr("class","node").attr("width",function(a){return a.width+
2*d.pad+2*d.margin}).attr("height",function(a){return a.height+2*d.pad+2*d.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);c=h.selectAll(".label").data(c.nodes).enter().append("text").attr("class","label").call(this.d3cola.drag);c.each(e.insertLinebreaks);k.append("title").text(function(a){return a.label});return new b.Cola_graph(k,f,a,c,this)};e.insertLinebreaks=function(c){var d=d3.select(this);c=c.label.split(" ");d.text("");for(var a=0;a<
c.length;a++){var b=c[a];b=d.append("tspan").text(b);b.attr("x",0).attr("dy","15").attr("font-size","12")}};return e}();b.Metabolic_pathway=f})(apps||(apps={}));
(function(b){var f=function(e){function c(){var d=null!==e&&e.apply(this,arguments)||this;d.assemblyKey="ko00001-assembly";d.canvas=new b.Metabolic_pathway;return d}__extends(c,e);Object.defineProperty(c.prototype,"appName",{get:function(){return"Pathway_explorer"},enumerable:!0,configurable:!0});c.prototype.init=function(){var d=$ts("@data:repository"),a=this,c=localStorage.getItem(this.assemblyKey);Strings.Empty(c)?$ts.get(d,function(d){a.saveCache(d);a.loadCache()}):a.loadCache();a.canvas.init()};
c.prototype.loadUITree=function(d){d=PathwayNavigator.parseJsTree(d);var a=$ts("@app:explorer");$("#"+a).jstree({core:{data:d.children}})};c.prototype.saveCache=function(d){var a=[],c=0;for(d=d.children;c<d.length;c++){var b=d[c],e=b.name.toLowerCase().replace(/\s+/ig,"_");a.push(e);localStorage.setItem(e,JSON.stringify(b))}localStorage.setItem(this.assemblyKey,JSON.stringify(a))};c.prototype.loadCache=function(){var c=localStorage.getItem(this.assemblyKey),a={name:"ko00001",children:[]},b=0;for(c=
JSON.parse(c);b<c.length;b++){var e=c[b];TypeScript.logging.log("load cache: "+e+"...");e=localStorage.getItem(e);a.children.push(JSON.parse(e))}this.loadUITree(a)};return c}(Bootstrap);b.PathwayExplorer=f})(apps||(apps={}));var PathwayNavigator;
(function(b){function f(c){var d=KEGG.brite.parseIDEntry(c.name),a=d.commonName||"KEGG";return isNullOrEmpty(c.children)?{id:d.id+"_"+ ++e,text:c.name,children:null,icon:"data:image/png;base64,"}:{id:d.id+
"_"+ ++e,text:a,children:$from(c.children).Select(f).ToArray(),icon:"data:image/png;base64,"}}
var e=1;b.parseJsTree=f})(PathwayNavigator||(PathwayNavigator={}));var dataAdapter;
(function(b){var f=function(){function b(c){void 0===c&&(c="#embeddedsvg");this.nodeLookup={};this.sbsvg=d3.select($ts(c)).select("svg");this.graph={nodes:[],links:[],constraints:[],groups:[]}}b.prototype.getGraph=function(){var c=this;this.loadNodes(this.sbsvg);for(var d=0,a=this.sbsvg.selectAll(".connector")[0];d<a.length;d++){var b=a[d],e=b.getAttribute("dunnart:srcid");b=b.getAttribute("dunnart:dstid");e in this.nodeLookup?b in this.nodeLookup?this.graph.links.push({source:this.nodeLookup[e].index,
target:this.nodeLookup[b].index}):console.warn("missing node of "+b):console.warn("missing node of "+e)}d=0;for(a=this.sbsvg.selectAll(".cluster")[0];d<a.length;d++)e=a[d],this.graph.groups.push({leaves:e.getAttribute("dunnart:contains").split(" ").map(function(a){return c.nodeLookup[a].index}),style:e.getAttribute("style"),padding:10});d={};a=0;for(e=this.sbsvg.selectAll("[relType=alignment]")[0];a<e.length;a++){var f=e[a];b=f.getAttribute("constraintid");var g=this.nodeLookup[f.getAttribute("objoneid")].index;
f=parseInt(f.getAttribute("alignmentpos"));g={node:g,offset:0};b in d?d[b].offsets.push(g):this.graph.constraints.push(d[b]={type:"alignment",offsets:[g],axis:1==f?"y":"x"})}return this.graph};b.prototype.loadNodes=function(c){var b=0,a=0;for(c=c.selectAll("rect.shape")[0];a<c.length;a++){var e=c[a];this.graph.nodes.push(this.nodeLookup[e.id]={label:e.getAttribute("dunnart:label"),dunnartid:e.id,index:b++,width:60,height:40,x:parseFloat(e.getAttribute("x")),y:parseFloat(e.getAttribute("y")),rx:"rx"in
e.attributes?parseFloat(e.getAttribute("rx")):5,ry:"ry"in e.attributes?parseFloat(e.getAttribute("ry")):5})}};return b}();b.parseDunnart=f})(dataAdapter||(dataAdapter={}));
