...
function(a){return c.d3cola.on("tick",c.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};b.prototype.loadGraph=function(c){var a=this,g=this.vis.append("g"),f=this.vis.append("g"),h=this.vis.append("g");console.log(JSON.stringify(c));this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",
5).attr("markerWidth",3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");g=g.selectAll(".group").data(c.groups).enter().append("rect").attr("rx",8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});h=h.selectAll(".link").data(c.links).enter().append("line").attr("class","link");var e=f.selectAll(".node").data(c.nodes).enter().append("rect").attr("class","node").attr("width",function(c){return c.width+
2*a.pad+2*a.margin}).attr("height",function(c){return c.height+2*a.pad+2*a.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);c=f.selectAll(".label").data(c.nodes).enter().append("text").attr("class","label").call(this.d3cola.drag);c.each(b.insertLinebreaks);e.append("title").text(function(a){return a.label});return new d.Cola_graph(e,h,g,c,this)};b.insertLinebreaks=function(c){var a=d3.select(this);c=c.label.split(" ");a.text("");for(var b=0;b<
c.length;b++){var f=c[b];f=a.append("tspan").text(f);f.attr("x",0).attr("dy","15").attr("font-size","12")}};return b}();d.Metabolic_pathway=b})(apps||(apps={}));
...
"flow"===a.itemType?go.Link.Normal:go.Link.Bezier;this.temporaryLink.path.stroke="flow"===a.itemType?"blue":"green";this.temporaryLink.path.strokeWidth="flow"===a.itemType?5:1;go.LinkingTool.prototype.doActivate.call(this)},"linkingTool.insertLink":function(b,g,d,e){c.myDiagram.model.setCategoryForLinkData(this.archetypeLinkData,a.itemType);this.archetypeLabelNodeData="flow"===a.itemType?{category:"valve"}:null;this.archetypeLinkData.text=a.itemType;return go.LinkingTool.prototype.insertLink.call(this,
b,g,d,e)},"clickCreatingTool.archetypeNodeData":{},"commandHandler.archetypeGroupData":{text:"Group",isGroup:!0,color:"blue"},"clickCreatingTool.isDoubleClick":!1,"clickCreatingTool.canStart":function(){return"node"===a.mode&&go.ClickCreatingTool.prototype.canStart.call(this)},"clickCreatingTool.insertPart":function(c){a.nodeCounter[a.itemType]+=1;var b="T"+a.itemType+a.nodeCounter[a.itemType];this.archetypeNodeData={key:b,category:a.itemType,label:b};return go.ClickCreatingTool.prototype.insertPart.call(this,
...
(function(d){function b(c){return c.replace(".","_").replace("-","_").replace("+","_").replace("*","_").replace("~","_")}var e=/[-]?\d+/ig;d.ModelPatch=function(c){for(var a=0,d=c.nodeDataArray;a<d.length;a++){var f=d[a];Strings.IsPattern(f.key.toString(),e)&&(f.key=b("T"+f.key))}a=0;for(d=c.linkDataArray;a<d.length;a++)if(f=d[a],Strings.IsPattern(f.from.toString(),e)&&(f.from=b("T"+f.from)),Strings.IsPattern(f.to.toString(),e)&&(f.to=b("T"+f.to)),!isNullOrEmpty(f.labelKeys))for(var h=0;h<f.labelKeys.length;h++)Strings.IsPattern(f.labelKeys[h].toString(),
e)&&(f.labelKeys[h]=b("T"+f.labelKeys[h]));return c}})(apps||(apps={}));
