...
(function(b){(function(b){b.paper_colors="#D02823 #0491d0 #88bb64 #15DBFF #583B73 #f2ce3f #8858BF #CCFF33 #fb5b44 #361f32 #DF2789 #396b1c".split(" ");b.translateToColaGraph=function(d){for(var c={groups:[],constraints:[],links:[],nodes:[]},a={},h={},f=0,e=d.nodeDataArray;f<e.length;f++){var g=e[f];if(!isNullOrUndefined(g.isGroup)&&g.isGroup){var k=b.paper_colors[Object.keys(h).length];h[g.key.toString()]={leaves:[],padding:10,style:"fill:"+k+";fill-opacity:0.31764700000000001;stroke:"+k+";stroke-opacity:1"}}}f=
0;for(e=d.nodeDataArray;f<e.length;f++)g=e[f],!isNullOrUndefined(g.isGroup)&&g.isGroup||"valve"==g.category||(k=g.loc,k=Strings.Empty(k)?[0,0]:k.split(/\s+/ig).map(function(a){return parseFloat(a)}),2>k.length&&(k=[0,0]),c.nodes.push({dunnartid:(c.nodes.length+1).toString(),height:40,index:c.nodes.length,label:g.label,rx:9,ry:9,width:60,x:k[0],y:k[1]}),a[g.key]=c.nodes.length-1,isNullOrUndefined(g.group)||h[g.group.toString()].leaves.push(c.nodes.length-1));g=0;for(d=d.linkDataArray;g<d.length;g++)f=
d[g],c.links.push({source:a[f.from],target:a[f.to]});for(var m in h)c.groups.push(h[m]);return c}})(b.translation||(b.translation={}))})(apps||(apps={}));
...
f.to=d(f.to),!isNullOrEmpty(f.labelKeys))for(var l=0;l<f.labelKeys.length;l++)Strings.IsPattern(f.labelKeys[l].toString(),e)&&(f.labelKeys[l]="T"+f.labelKeys[l]),f.labelKeys[l]=d(f.labelKeys[l]);return c}})(apps||(apps={}));
...
(function(b){var d=function(d){function c(){return null!==d&&d.apply(this,arguments)||this}__extends(c,d);Object.defineProperty(c.prototype,"appName",{get:function(){return"kegg_network"},enumerable:!0,configurable:!0});c.prototype.init=function(){var a=PathwayNavigator.parseJsTree(b.PathwayExplorer.loadKEGGTree()),d=[];c.createSet(a,d);console.log(d)};c.createSet=function(a,c){var b=0;for(a=a.children;b<a.length;b++){var d=a[b];isNullOrUndefined(d.children)?c.push(d):this.createSet(d,c)}};return c}(Bootstrap);
b.KEGGNetwork=d})(apps||(apps={}));
...
target:this.nodeLookup[f].index}):console.warn("missing node of "+f):console.warn("missing node of "+e)}a=0;for(d=this.sbsvg.selectAll(".cluster")[0];a<d.length;a++)e=d[a],this.graph.groups.push({leaves:e.getAttribute("dunnart:contains").split(" ").map(function(a){return b.nodeLookup[a].index}),style:e.getAttribute("style"),padding:10});a={};d=0;for(e=this.sbsvg.selectAll("[relType=alignment]")[0];d<e.length;d++){var g=e[d];f=g.getAttribute("constraintid");var k=this.nodeLookup[g.getAttribute("objoneid")].index;
