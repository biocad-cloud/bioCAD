var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.SIMPLE_FROUND_POLYFILL=!1;$jscomp.defineProperty=$jscomp.ASSUME_ES5||"function"==typeof Object.defineProperties?Object.defineProperty:function(c,b,f){c!=Array.prototype&&c!=Object.prototype&&(c[b]=f.value)};$jscomp.getGlobal=function(c){return"undefined"!=typeof window&&window===c?c:"undefined"!=typeof global&&null!=global?global:c};$jscomp.global=$jscomp.getGlobal(this);
$jscomp.polyfill=function(c,b,f,d){if(b){f=$jscomp.global;c=c.split(".");for(d=0;d<c.length-1;d++){var a=c[d];a in f||(f[a]={});f=f[a]}c=c[c.length-1];d=f[c];b=b(d);b!=d&&null!=b&&$jscomp.defineProperty(f,c,{configurable:!0,writable:!0,value:b})}};$jscomp.underscoreProtoCanBeSet=function(){var c={a:!0},b={};try{return b.__proto__=c,b.a}catch(f){}return!1};
$jscomp.setPrototypeOf="function"==typeof Object.setPrototypeOf?Object.setPrototypeOf:$jscomp.underscoreProtoCanBeSet()?function(c,b){c.__proto__=b;if(c.__proto__!==b)throw new TypeError(c+" is not extensible");return c}:null;$jscomp.polyfill("Object.setPrototypeOf",function(c){return c||$jscomp.setPrototypeOf},"es6","es5");
var __extends=this&&this.__extends||function(){var c=function(b,f){c=Object.setPrototypeOf||{__proto__:[]}instanceof Array&&function(d,a){d.__proto__=a}||function(d,a){for(var g in a)a.hasOwnProperty(g)&&(d[g]=a[g])};return c(b,f)};return function(b,f){function d(){this.constructor=b}c(b,f);b.prototype=null===f?Object.create(f):(d.prototype=f.prototype,new d)}}(),biodeep;
(function(c){(function(b){b.start=function(){Router.AddAppHandler(new apps.PathwayExplorer);Router.AddAppHandler(new apps.FlowEditor);Router.RunApp()}})(c.app||(c.app={}))})(biodeep||(biodeep={}));var __indexOf=Array.prototype.indexOf||function(c){for(var b=0,f=this.length;b<f;b++)if(b in this&&this[b]===c)return b;return-1};$ts.mode=Modes.debug;$ts(biodeep.app.start);var apps;
(function(c){var b=function(){function b(d,a,g,e,b){this.node=d;this.link=a;this.group=g;this.label=e;this.vm=b;console.log(this)}b.prototype.tick=function(){var d=this.vm,a=this;return function(){a.node.each(function(a){return a.innerBounds=a.bounds.inflate(-d.margin)});a.link.each(function(a){return a.route=cola.makeEdgeBetween(a.source.innerBounds,a.target.innerBounds,5)});a.link.attr("x1",function(a){return a.route.sourceIntersection.x}).attr("y1",function(a){return a.route.sourceIntersection.y}).attr("x2",
function(a){return a.route.arrowStart.x}).attr("y2",function(a){return a.route.arrowStart.y});a.label.each(function(a){var e=this.getBBox();a.width=e.width+2*d.margin+8;a.height=e.height+2*d.margin+8});a.node.attr("x",function(a){return a.innerBounds.x}).attr("y",function(a){return a.innerBounds.y}).attr("width",function(a){return a.innerBounds.width()}).attr("height",function(a){return a.innerBounds.height()});a.group.attr("x",function(a){return a.bounds.x}).attr("y",function(a){return a.bounds.y}).attr("width",
function(a){return a.bounds.width()}).attr("height",function(a){return a.bounds.height()});a.label.attr("transform",function(d){return a.transform(d)})}};b.prototype.transform=function(d){return"translate("+d.x+this.vm.margin+", "+(d.y+this.vm.margin-d.height/2)+")"};return b}();c.Cola_graph=b})(apps||(apps={}));
(function(c){var b=function(){function b(){this.width=800;this.height=520;this.margin=6;this.pad=12;this.width=parseFloat($ts("@width"));this.height=parseFloat($ts("@height"))}b.prototype.redraw=function(){this.vis.attr("transform","translate("+d3.event.translate+") scale("+d3.event.scale+")")};b.prototype.savePng=function(){saveSvgAsPng($ts("#canvas").childNodes.item(0),"pathway.png")};b.prototype.init=function(){var d=this,a=$ts("@url:graph");this.d3cola=cola.d3adaptor(d3).linkDistance(60).avoidOverlaps(!0).size([this.width,
this.height]);this.outer=d3.select($ts("#canvas")).append("svg").attr("width",this.width).attr("height",this.height).attr("pointer-events","all");this.outer.append("rect").attr("class","background").attr("width","100%").attr("height","100%").call(d3.behavior.zoom().on("zoom",function(){return d.redraw()}));this.vis=this.outer.append("g").attr("transform","translate(80,80) scale(0.7)");if("#"==a.charAt(0))this.d3cola.on("tick",this.loadGraph((new dataAdapter.parseDunnart(a)).getGraph()).tick());else $ts.getText(a,
function(a){return d.d3cola.on("tick",d.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};b.prototype.loadGraph=function(d){var a=this,g=this.vis.append("g"),e=this.vis.append("g"),h=this.vis.append("g");console.log(JSON.stringify(d));this.d3cola.nodes(d.nodes).links(d.links).groups(d.groups).constraints(d.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",
5).attr("markerWidth",3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");g=g.selectAll(".group").data(d.groups).enter().append("rect").attr("rx",8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});h=h.selectAll(".link").data(d.links).enter().append("line").attr("class","link");var f=e.selectAll(".node").data(d.nodes).enter().append("rect").attr("class","node").attr("width",function(e){return e.width+
2*a.pad+2*a.margin}).attr("height",function(e){return e.height+2*a.pad+2*a.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);d=e.selectAll(".label").data(d.nodes).enter().append("text").attr("class","label").call(this.d3cola.drag);d.each(b.insertLinebreaks);f.append("title").text(function(a){return a.label});return new c.Cola_graph(f,h,g,d,this)};b.insertLinebreaks=function(d){var a=d3.select(this);d=d.label.split(" ");a.text("");for(var b=0;b<
d.length;b++){var e=d[b];e=a.append("tspan").text(e);e.attr("x",0).attr("dy","15").attr("font-size","12")}};return b}();c.Metabolic_pathway=b})(apps||(apps={}));
(function(c){var b=function(b){function d(){var a=null!==b&&b.apply(this,arguments)||this;a.SD={mode:"pointer",itemType:"pointer",nodeCounter:{stock:0,cloud:0,variable:0,valve:0}};return a}__extends(d,b);Object.defineProperty(d.prototype,"appName",{get:function(){return"FlowEditor"},enumerable:!0,configurable:!0});d.prototype.config=function(){var a=this.SD,d=this;return{"undoManager.isEnabled":!0,allowLink:!1,"animationManager.isEnabled":!1,"linkingTool.portGravity":0,"linkingTool.doActivate":function(){this.temporaryLink.curve=
"flow"===a.itemType?go.Link.Normal:go.Link.Bezier;this.temporaryLink.path.stroke="flow"===a.itemType?"blue":"green";this.temporaryLink.path.strokeWidth="flow"===a.itemType?5:1;go.LinkingTool.prototype.doActivate.call(this)},"linkingTool.insertLink":function(e,b,g,c){d.myDiagram.model.setCategoryForLinkData(this.archetypeLinkData,a.itemType);this.archetypeLabelNodeData="flow"===a.itemType?{category:"valve"}:null;this.archetypeLinkData.text=a.itemType;return go.LinkingTool.prototype.insertLink.call(this,
e,b,g,c)},"clickCreatingTool.archetypeNodeData":{},"commandHandler.archetypeGroupData":{text:"Group",isGroup:!0,color:"blue"},"clickCreatingTool.isDoubleClick":!1,"clickCreatingTool.canStart":function(){return"node"===a.mode&&go.ClickCreatingTool.prototype.canStart.call(this)},"clickCreatingTool.insertPart":function(e){a.nodeCounter[a.itemType]+=1;var d=a.itemType+a.nodeCounter[a.itemType];this.archetypeNodeData={key:d,category:a.itemType,label:d};return go.ClickCreatingTool.prototype.insertPart.call(this,
e)}}};d.prototype.init=function(){var a=go.GraphObject.make,d=this.SD,e=this.myDiagram=a(go.Diagram,"myDiagram",this.config());e.toolManager.mouseMoveTools.insertAt(0,new NodeLabelDraggingTool);e.addDiagramListener("Modified",function(a){a=document.getElementById("SaveButton");var d=document.title.indexOf("*");a&&(a.disabled=!e.isModified);e.isModified?0>d&&(document.title+="*"):0<=d&&(document.title=document.title.substr(0,d))});e.addDiagramListener("LinkDrawn",function(a){var b=a.subject;"flow"===
b.category&&(e.startTransaction("updateNode"),d.nodeCounter.valve+=1,a="flow"+d.nodeCounter.valve,b=b.labelNodes.first(),e.model.setDataProperty(b.data,"label",a),e.commitTransaction("updateNode"))});this.buildTemplates();this.load()};d.prototype.buildTemplates=function(){var a=go.GraphObject.make,b=this.myDiagram;b.nodeTemplateMap.add("stock",a(go.Node,c.EditorTemplates.nodeStyle(),a(go.Shape,c.EditorTemplates.shapeStyle(),{desiredSize:new go.Size(50,30)}),a(go.TextBlock,c.EditorTemplates.textStyle(),
{_isNodeLabel:!0,alignment:new go.Spot(.5,.5,0,30)},(new go.Binding("alignment","label_offset",go.Spot.parse)).makeTwoWay(go.Spot.stringify))));b.nodeTemplateMap.add("cloud",a(go.Node,c.EditorTemplates.nodeStyle(),a(go.Shape,c.EditorTemplates.shapeStyle(),{figure:"Cloud",desiredSize:new go.Size(35,35)})));b.nodeTemplateMap.add("valve",a(go.Node,c.EditorTemplates.nodeStyle(),{movable:!1,layerName:"Foreground",alignmentFocus:go.Spot.None},a(go.Shape,c.EditorTemplates.shapeStyle(),{figure:"Ellipse",
desiredSize:new go.Size(20,20)}),a(go.TextBlock,c.EditorTemplates.textStyle(),{_isNodeLabel:!0,alignment:new go.Spot(.5,.5,0,20)},(new go.Binding("alignment","label_offset",go.Spot.parse)).makeTwoWay(go.Spot.stringify))));b.nodeTemplateMap.add("variable",a(go.Node,c.EditorTemplates.nodeStyle(),{type:go.Panel.Auto},a(go.TextBlock,c.EditorTemplates.textStyle(),{isMultiline:!1}),a(go.Shape,c.EditorTemplates.shapeStyle(),{isPanelMain:!0,stroke:null,fill:"transparent"})));b.linkTemplateMap.add("flow",
a(go.Link,{toShortLength:8},a(go.Shape,{stroke:"blue",strokeWidth:5}),a(go.Shape,{fill:"blue",stroke:null,toArrow:"Standard",scale:2.5})));b.linkTemplateMap.add("influence",a(go.Link,{curve:go.Link.Bezier,toShortLength:8},a(go.Shape,{stroke:"green",strokeWidth:1.5}),a(go.Shape,{fill:"green",stroke:null,toArrow:"Standard",scale:1.5})));b.groupTemplate=a(go.Group,"Vertical",{selectionObjectName:"PANEL",ungroupable:!0},a(go.TextBlock,{font:"bold 19px sans-serif",isMultiline:!1,editable:!0},(new go.Binding("text",
"text")).makeTwoWay(),new go.Binding("stroke","color")),a(go.Panel,"Auto",{name:"PANEL"},a(go.Shape,"Rectangle",{fill:"rgba(128,128,128,0.2)",stroke:"gray",strokeWidth:3,portId:"",cursor:"pointer",fromLinkable:!0,fromLinkableSelfNode:!0,fromLinkableDuplicates:!0,toLinkable:!0,toLinkableSelfNode:!0,toLinkableDuplicates:!0}),a(go.Placeholder,{margin:10,background:"transparent"})),{toolTip:a("ToolTip",a(go.TextBlock,{margin:4},(new go.Binding("text","",d.groupInfo)).ofObject()))})};d.groupInfo=function(a){a=
a.adornedPart;var d=a.memberParts.count,b=0;a.memberParts.each(function(a){a instanceof go.Link&&b++});return"Group "+a.data.key+": "+a.data.text+"\n"+d+" members including "+b+" links"};d.prototype.setMode=function(a,d){var b=this.myDiagram,c=this.SD;b.startTransaction();document.getElementById(c.itemType).className=c.mode+"_normal";document.getElementById(d).className=a+"_selected";c.mode=a;c.itemType=d;"pointer"===a?(b.allowLink=!1,b.nodes.each(function(a){return a.port.cursor=""})):"node"===a?
(b.allowLink=!1,b.nodes.each(function(a){return a.port.cursor=""})):"link"===a&&(b.allowLink=!0,b.nodes.each(function(a){return a.port.cursor="pointer"}));b.commitTransaction("mode changed")};d.prototype.save_click=function(){this.dosave()};d.prototype.load=function(){var a=this;$ts.getText("@api:load?model_id="+$ts("@data:model_id"),function(b){b=c.ModelPatch(JSON.parse(b));b=JSON.stringify(b);a.myDiagram.model=go.Model.fromJson(b)})};d.prototype.dosave=function(a){void 0===a&&(a=null);var b=this.myDiagram,
d=b.model.toJson();d={guid:$ts("@data:model_id"),model:c.ModelPatch(JSON.parse(d)),type:"dynamics"};b.isModified=!1;$ts.post("@api:save",d,function(b){0!=b.code?console.error(b.info):isNullOrUndefined(b.info)||isNullOrUndefined(a)||a(b.info)})};d.prototype.pointer_click=function(){this.setMode("pointer","pointer")};d.prototype.stock_click=function(){this.setMode("node","stock")};d.prototype.cloud_click=function(){this.setMode("node","cloud")};d.prototype.variable_click=function(){this.setMode("node",
"variable")};d.prototype.flow_click=function(){this.setMode("link","flow")};d.prototype.influence_click=function(){this.setMode("link","influence")};d.prototype.run_click=function(){var a=this;this.dosave(function(b){return a.doRunModel(b)})};d.prototype.doRunModel=function(a){$ts.post("@api:run/"+a,{guid:a},function(a){0!=a.code?console.error(a.info):$goto(a.url)})};return d}(Bootstrap);c.FlowEditor=b})(apps||(apps={}));
(function(c){function b(b){return b?b.toString().replace(".","_").replace("-","_").replace("+","_").replace("*","_").replace("~","_"):null}var f=/[-]?\d+/ig;c.ModelPatch=function(d){for(var a=0,c=d.nodeDataArray;a<c.length;a++){var e=c[a];Strings.IsPattern(e.key.toString(),f)&&(e.key="T"+e.key);e.group&&Strings.IsPattern(e.group.toString(),f)&&(e.group="T"+e.group);e.key=b(e.key);e.group=b(e.group);e.group||delete e.group}a=0;for(c=d.linkDataArray;a<c.length;a++)if(e=c[a],Strings.IsPattern(e.from.toString(),
f)&&(e.from="T"+e.from),Strings.IsPattern(e.to.toString(),f)&&(e.to="T"+e.to),e.from=b(e.from),e.to=b(e.to),!isNullOrEmpty(e.labelKeys))for(var h=0;h<e.labelKeys.length;h++)Strings.IsPattern(e.labelKeys[h].toString(),f)&&(e.labelKeys[h]="T"+e.labelKeys[h]),e.labelKeys[h]=b(e.labelKeys[h]);return d}})(apps||(apps={}));
(function(c){(function(b){b.nodeStyle=function(){return[{type:go.Panel.Spot,layerName:"Background",locationObjectName:"SHAPE",selectionObjectName:"SHAPE",locationSpot:go.Spot.Center},(new go.Binding("location","loc",go.Point.parse)).makeTwoWay(go.Point.stringify)]};b.shapeStyle=function(){return{name:"SHAPE",stroke:"black",fill:"#f0f0f0",portId:"",fromLinkable:!0,toLinkable:!0}};b.textStyle=function(){return[{font:"bold 11pt helvetica, bold arial, sans-serif",margin:2,editable:!0},(new go.Binding("text",
"label")).makeTwoWay()]}})(c.EditorTemplates||(c.EditorTemplates={}))})(apps||(apps={}));
(function(c){var b=function(b){function d(){var a=null!==b&&b.apply(this,arguments)||this;a.assemblyKey="ko00001-assembly";a.canvas=new c.Metabolic_pathway;return a}__extends(d,b);Object.defineProperty(d.prototype,"appName",{get:function(){return"Pathway_explorer"},enumerable:!0,configurable:!0});d.prototype.init=function(){var a=$ts("@data:repository"),b=this,d=localStorage.getItem(this.assemblyKey);Strings.Empty(d)?$ts.get(a,function(a){b.saveCache(a);b.loadCache()}):b.loadCache();b.canvas.init()};
d.prototype.loadUITree=function(a){a=PathwayNavigator.parseJsTree(a);var b=$ts("@app:explorer");$("#"+b).jstree({core:{data:a.children},plugins:["contextmenu"],contextmenu:{items:{add_reactor:{label:"Add Reactor",action:d.addReactor}}}})};d.addReactor=function(a){a.reference[0].id.match(/K\d{4,}/g)};d.prototype.saveCache=function(a){var b=[],d=0;for(a=a.children;d<a.length;d++){var c=a[d],f=c.name.toLowerCase().replace(/\s+/ig,"_");b.push(f);localStorage.setItem(f,JSON.stringify(c))}localStorage.setItem(this.assemblyKey,
JSON.stringify(b))};d.prototype.loadCache=function(){var a=localStorage.getItem(this.assemblyKey),b={name:"ko00001",children:[]},d=0;for(a=JSON.parse(a);d<a.length;d++){var c=a[d];TypeScript.logging.log("load cache: "+c+"...");c=localStorage.getItem(c);b.children.push(JSON.parse(c))}this.loadUITree(b)};return d}(Bootstrap);c.PathwayExplorer=b})(apps||(apps={}));var PathwayNavigator;
(function(c){function b(d){var a=KEGG.brite.parseIDEntry(d.name),c=a.commonName||"KEGG";return isNullOrEmpty(d.children)?{id:a.id+"_"+ ++f,text:d.name,children:null,icon:"data:image/png;base64,"}:
{id:a.id+"_"+ ++f,text:c,children:$from(d.children).Select(b).ToArray(),icon:"data:image/png;base64,"}}
var f=1;c.parseJsTree=b})(PathwayNavigator||(PathwayNavigator={}));
var Graph=function(){function c(b){void 0===b&&(b=null);this.last_index=0;isNullOrUndefined(b)||(this.nodes=b.nodes,this.links=b.links)}c.prototype.objectify=function(){for(var b=this,c=b.links,d=[],a=function(a,e){var f=c[a];d.push(function(){var a;var d=b.nodes;var c=[];var e=0;for(a=d.length;e<a;e++){var g=d[e];f.source===g.id?f.source=g:f.target===g.id?f.target=g:c.push(void 0)}return c}())},g=0,e=c.length;g<e;g++)a(g,e);return d};c.prototype.remove=function(b){if(0<=__indexOf.call(this.nodes,
b))return this.nodes=this.nodes.filter(function(c){return c!==b}),this.links=this.links.filter(function(c){return c.source.id!==b.id&&c.target.id!==b.id});if(0<=__indexOf.call(this.links,b))return this.links=this.links.filter(function(c){return c!==b})};c.prototype.add_node=function(b){b={dunnartid:(this.last_index++).toString(),x:0,y:0,type:b};this.nodes.push(b);return b};c.prototype.add_link=function(b,c){if(b===c)return null;for(var d=this.links,a=0,f=d.length;a<f;a++){var e=d[a];if(e.source===
b&&e.target===c)return null}b={source:b,target:c};this.links.push(b);return b};return c}(),dataAdapter;
(function(c){var b=function(){function b(b){void 0===b&&(b="#embeddedsvg");this.nodeLookup={};this.sbsvg=d3.select($ts(b)).select("svg");this.graph={nodes:[],links:[],constraints:[],groups:[]}}b.prototype.getGraph=function(){var b=this;this.loadNodes(this.sbsvg);for(var a=0,c=this.sbsvg.selectAll(".connector")[0];a<c.length;a++){var e=c[a],f=e.getAttribute("dunnart:srcid");e=e.getAttribute("dunnart:dstid");f in this.nodeLookup?e in this.nodeLookup?this.graph.links.push({source:this.nodeLookup[f].index,
target:this.nodeLookup[e].index}):console.warn("missing node of "+e):console.warn("missing node of "+f)}a=0;for(c=this.sbsvg.selectAll(".cluster")[0];a<c.length;a++)f=c[a],this.graph.groups.push({leaves:f.getAttribute("dunnart:contains").split(" ").map(function(a){return b.nodeLookup[a].index}),style:f.getAttribute("style"),padding:10});a={};c=0;for(f=this.sbsvg.selectAll("[relType=alignment]")[0];c<f.length;c++){var k=f[c];e=k.getAttribute("constraintid");var l=this.nodeLookup[k.getAttribute("objoneid")].index;
k=parseInt(k.getAttribute("alignmentpos"));l={node:l,offset:0};e in a?a[e].offsets.push(l):this.graph.constraints.push(a[e]={type:"alignment",offsets:[l],axis:1==k?"y":"x"})}return this.graph};b.prototype.loadNodes=function(b){var a=0,d=0;for(b=b.selectAll("rect.shape")[0];d<b.length;d++){var c=b[d];this.graph.nodes.push(this.nodeLookup[c.id]={label:c.getAttribute("dunnart:label"),dunnartid:c.id,index:a++,width:60,height:40,x:parseFloat(c.getAttribute("x")),y:parseFloat(c.getAttribute("y")),rx:"rx"in
c.attributes?parseFloat(c.getAttribute("rx")):5,ry:"ry"in c.attributes?parseFloat(c.getAttribute("ry")):5})}};return b}();c.parseDunnart=b})(dataAdapter||(dataAdapter={}));
