var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.SIMPLE_FROUND_POLYFILL=!1;$jscomp.defineProperty=$jscomp.ASSUME_ES5||"function"==typeof Object.defineProperties?Object.defineProperty:function(d,b,f){d!=Array.prototype&&d!=Object.prototype&&(d[b]=f.value)};$jscomp.getGlobal=function(d){return"undefined"!=typeof window&&window===d?d:"undefined"!=typeof global&&null!=global?global:d};$jscomp.global=$jscomp.getGlobal(this);
$jscomp.polyfill=function(d,b,f,c){if(b){f=$jscomp.global;d=d.split(".");for(c=0;c<d.length-1;c++){var a=d[c];a in f||(f[a]={});f=f[a]}d=d[d.length-1];c=f[d];b=b(c);b!=c&&null!=b&&$jscomp.defineProperty(f,d,{configurable:!0,writable:!0,value:b})}};$jscomp.underscoreProtoCanBeSet=function(){var d={a:!0},b={};try{return b.__proto__=d,b.a}catch(f){}return!1};
$jscomp.setPrototypeOf="function"==typeof Object.setPrototypeOf?Object.setPrototypeOf:$jscomp.underscoreProtoCanBeSet()?function(d,b){d.__proto__=b;if(d.__proto__!==b)throw new TypeError(d+" is not extensible");return d}:null;$jscomp.polyfill("Object.setPrototypeOf",function(d){return d||$jscomp.setPrototypeOf},"es6","es5");
var __extends=this&&this.__extends||function(){var d=function(b,f){d=Object.setPrototypeOf||{__proto__:[]}instanceof Array&&function(c,a){c.__proto__=a}||function(c,a){for(var g in a)a.hasOwnProperty(g)&&(c[g]=a[g])};return d(b,f)};return function(b,f){function c(){this.constructor=b}d(b,f);b.prototype=null===f?Object.create(f):(c.prototype=f.prototype,new c)}}(),biodeep;
(function(d){(function(b){b.start=function(){Router.AddAppHandler(new apps.PathwayExplorer);Router.AddAppHandler(new apps.FlowEditor);Router.RunApp()}})(d.app||(d.app={}))})(biodeep||(biodeep={}));var __indexOf=Array.prototype.indexOf||function(d){for(var b=0,f=this.length;b<f;b++)if(b in this&&this[b]===d)return b;return-1};$ts.mode=Modes.debug;$ts(biodeep.app.start);var apps;
(function(d){var b=function(){function b(c,a,g,e,b){this.node=c;this.link=a;this.group=g;this.label=e;this.vm=b;console.log(this)}b.prototype.tick=function(){var c=this.vm,a=this;return function(){a.node.each(function(a){return a.innerBounds=a.bounds.inflate(-c.margin)});a.link.each(function(a){return a.route=cola.makeEdgeBetween(a.source.innerBounds,a.target.innerBounds,5)});a.link.attr("x1",function(a){return a.route.sourceIntersection.x}).attr("y1",function(a){return a.route.sourceIntersection.y}).attr("x2",
function(a){return a.route.arrowStart.x}).attr("y2",function(a){return a.route.arrowStart.y});a.label.each(function(a){var g=this.getBBox();a.width=g.width+2*c.margin+8;a.height=g.height+2*c.margin+8});a.node.attr("x",function(a){return a.innerBounds.x}).attr("y",function(a){return a.innerBounds.y}).attr("width",function(a){return a.innerBounds.width()}).attr("height",function(a){return a.innerBounds.height()});a.group.attr("x",function(a){return a.bounds.x}).attr("y",function(a){return a.bounds.y}).attr("width",
function(a){return a.bounds.width()}).attr("height",function(a){return a.bounds.height()});a.label.attr("transform",function(c){return a.transform(c)})}};b.prototype.transform=function(c){return"translate("+c.x+this.vm.margin+", "+(c.y+this.vm.margin-c.height/2)+")"};return b}();d.Cola_graph=b})(apps||(apps={}));
(function(d){var b=function(){function b(){this.width=800;this.height=520;this.margin=6;this.pad=12;this.width=parseFloat($ts("@width"));this.height=parseFloat($ts("@height"))}b.prototype.redraw=function(){this.vis.attr("transform","translate("+d3.event.translate+") scale("+d3.event.scale+")")};b.prototype.savePng=function(){saveSvgAsPng($ts("#canvas").childNodes.item(0),"pathway.png")};b.prototype.init=function(){var c=this,a=$ts("@url:graph");this.d3cola=cola.d3adaptor(d3).linkDistance(60).avoidOverlaps(!0).size([this.width,
this.height]);this.outer=d3.select($ts("#canvas")).append("svg").attr("width",this.width).attr("height",this.height).attr("pointer-events","all");this.outer.append("rect").attr("class","background").attr("width","100%").attr("height","100%").call(d3.behavior.zoom().on("zoom",function(){return c.redraw()}));this.vis=this.outer.append("g").attr("transform","translate(80,80) scale(0.7)");if("#"==a.charAt(0))this.d3cola.on("tick",this.loadGraph((new dataAdapter.parseDunnart(a)).getGraph()).tick());else $ts.getText(a,
function(a){return c.d3cola.on("tick",c.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};b.prototype.loadGraph=function(c){var a=this,g=this.vis.append("g"),e=this.vis.append("g"),h=this.vis.append("g");console.log(JSON.stringify(c));this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",
5).attr("markerWidth",3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");g=g.selectAll(".group").data(c.groups).enter().append("rect").attr("rx",8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});h=h.selectAll(".link").data(c.links).enter().append("line").attr("class","link");var f=e.selectAll(".node").data(c.nodes).enter().append("rect").attr("class","node").attr("width",function(c){return c.width+
2*a.pad+2*a.margin}).attr("height",function(c){return c.height+2*a.pad+2*a.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);c=e.selectAll(".label").data(c.nodes).enter().append("text").attr("class","label").call(this.d3cola.drag);c.each(b.insertLinebreaks);f.append("title").text(function(a){return a.label});return new d.Cola_graph(f,h,g,c,this)};b.insertLinebreaks=function(c){var a=d3.select(this);c=c.label.split(" ");a.text("");for(var b=0;b<
c.length;b++){var e=c[b];e=a.append("tspan").text(e);e.attr("x",0).attr("dy","15").attr("font-size","12")}};return b}();d.Metabolic_pathway=b})(apps||(apps={}));
(function(d){var b=function(b){function c(){var a=null!==b&&b.apply(this,arguments)||this;a.SD={mode:"pointer",itemType:"pointer",nodeCounter:{stock:0,cloud:0,variable:0,valve:0}};return a}__extends(c,b);Object.defineProperty(c.prototype,"appName",{get:function(){return"FlowEditor"},enumerable:!0,configurable:!0});c.prototype.config=function(){var a=this.SD,c=this;return{"undoManager.isEnabled":!0,allowLink:!1,"animationManager.isEnabled":!1,"linkingTool.portGravity":0,"linkingTool.doActivate":function(){this.temporaryLink.curve=
"flow"===a.itemType?go.Link.Normal:go.Link.Bezier;this.temporaryLink.path.stroke="flow"===a.itemType?"blue":"green";this.temporaryLink.path.strokeWidth="flow"===a.itemType?5:1;go.LinkingTool.prototype.doActivate.call(this)},"linkingTool.insertLink":function(b,g,d,f){c.myDiagram.model.setCategoryForLinkData(this.archetypeLinkData,a.itemType);this.archetypeLabelNodeData="flow"===a.itemType?{category:"valve"}:null;this.archetypeLinkData.text=a.itemType;return go.LinkingTool.prototype.insertLink.call(this,
b,g,d,f)},"clickCreatingTool.archetypeNodeData":{},"commandHandler.archetypeGroupData":{text:"Group",isGroup:!0,color:"blue"},"clickCreatingTool.isDoubleClick":!1,"clickCreatingTool.canStart":function(){return"node"===a.mode&&go.ClickCreatingTool.prototype.canStart.call(this)},"clickCreatingTool.insertPart":function(c){a.nodeCounter[a.itemType]+=1;var b=a.itemType+a.nodeCounter[a.itemType];this.archetypeNodeData={key:b,category:a.itemType,label:b};return go.ClickCreatingTool.prototype.insertPart.call(this,
c)}}};c.prototype.init=function(){var a=go.GraphObject.make,c=this.SD,b=this.myDiagram=a(go.Diagram,"myDiagram",this.config());b.toolManager.mouseMoveTools.insertAt(0,new NodeLabelDraggingTool);b.addDiagramListener("Modified",function(a){a=document.getElementById("SaveButton");var c=document.title.indexOf("*");a&&(a.disabled=!b.isModified);b.isModified?0>c&&(document.title+="*"):0<=c&&(document.title=document.title.substr(0,c))});b.addDiagramListener("LinkDrawn",function(a){var e=a.subject;"flow"===
e.category&&(b.startTransaction("updateNode"),c.nodeCounter.valve+=1,a="flow"+c.nodeCounter.valve,e=e.labelNodes.first(),b.model.setDataProperty(e.data,"label",a),b.commitTransaction("updateNode"))});this.buildTemplates();this.load()};c.prototype.buildTemplates=function(){var a=go.GraphObject.make,b=this.myDiagram;b.nodeTemplateMap.add("stock",a(go.Node,d.EditorTemplates.nodeStyle(),a(go.Shape,d.EditorTemplates.shapeStyle(),{desiredSize:new go.Size(50,30)}),a(go.TextBlock,d.EditorTemplates.textStyle(),
{_isNodeLabel:!0,alignment:new go.Spot(.5,.5,0,30)},(new go.Binding("alignment","label_offset",go.Spot.parse)).makeTwoWay(go.Spot.stringify))));b.nodeTemplateMap.add("cloud",a(go.Node,d.EditorTemplates.nodeStyle(),a(go.Shape,d.EditorTemplates.shapeStyle(),{figure:"Cloud",desiredSize:new go.Size(35,35)})));b.nodeTemplateMap.add("valve",a(go.Node,d.EditorTemplates.nodeStyle(),{movable:!1,layerName:"Foreground",alignmentFocus:go.Spot.None},a(go.Shape,d.EditorTemplates.shapeStyle(),{figure:"Ellipse",
desiredSize:new go.Size(20,20)}),a(go.TextBlock,d.EditorTemplates.textStyle(),{_isNodeLabel:!0,alignment:new go.Spot(.5,.5,0,20)},(new go.Binding("alignment","label_offset",go.Spot.parse)).makeTwoWay(go.Spot.stringify))));b.nodeTemplateMap.add("variable",a(go.Node,d.EditorTemplates.nodeStyle(),{type:go.Panel.Auto},a(go.TextBlock,d.EditorTemplates.textStyle(),{isMultiline:!1}),a(go.Shape,d.EditorTemplates.shapeStyle(),{isPanelMain:!0,stroke:null,fill:"transparent"})));b.linkTemplateMap.add("flow",
a(go.Link,{toShortLength:8},a(go.Shape,{stroke:"blue",strokeWidth:5}),a(go.Shape,{fill:"blue",stroke:null,toArrow:"Standard",scale:2.5})));b.linkTemplateMap.add("influence",a(go.Link,{curve:go.Link.Bezier,toShortLength:8},a(go.Shape,{stroke:"green",strokeWidth:1.5}),a(go.Shape,{fill:"green",stroke:null,toArrow:"Standard",scale:1.5})));b.groupTemplate=a(go.Group,"Vertical",{selectionObjectName:"PANEL",ungroupable:!0},a(go.TextBlock,{font:"bold 19px sans-serif",isMultiline:!1,editable:!0},(new go.Binding("text",
"text")).makeTwoWay(),new go.Binding("stroke","color")),a(go.Panel,"Auto",{name:"PANEL"},a(go.Shape,"Rectangle",{fill:"rgba(128,128,128,0.2)",stroke:"gray",strokeWidth:3,portId:"",cursor:"pointer",fromLinkable:!0,fromLinkableSelfNode:!0,fromLinkableDuplicates:!0,toLinkable:!0,toLinkableSelfNode:!0,toLinkableDuplicates:!0}),a(go.Placeholder,{margin:10,background:"transparent"})),{toolTip:a("ToolTip",a(go.TextBlock,{margin:4},(new go.Binding("text","",c.groupInfo)).ofObject()))})};c.groupInfo=function(a){a=
a.adornedPart;var c=a.memberParts.count,b=0;a.memberParts.each(function(a){a instanceof go.Link&&b++});return"Group "+a.data.key+": "+a.data.text+"\n"+c+" members including "+b+" links"};c.prototype.setMode=function(a,c){var b=this.myDiagram,d=this.SD;b.startTransaction();document.getElementById(d.itemType).className=d.mode+"_normal";document.getElementById(c).className=a+"_selected";d.mode=a;d.itemType=c;"pointer"===a?(b.allowLink=!1,b.nodes.each(function(a){return a.port.cursor=""})):"node"===a?
(b.allowLink=!1,b.nodes.each(function(a){return a.port.cursor=""})):"link"===a&&(b.allowLink=!0,b.nodes.each(function(a){return a.port.cursor="pointer"}));b.commitTransaction("mode changed")};c.prototype.save_click=function(){this.dosave()};c.prototype.load=function(){var a=this;$ts.getText("@api:load?model_id="+$ts("@data:model_id"),function(b){b=d.ModelPatch(JSON.parse(b));b=JSON.stringify(b);a.myDiagram.model=go.Model.fromJson(b)})};c.prototype.dosave=function(a){void 0===a&&(a=null);var b=this.myDiagram,
c=b.model.toJson();c={guid:$ts("@data:model_id"),model:d.ModelPatch(JSON.parse(c)),type:"dynamics"};b.isModified=!1;$ts.post("@api:save",c,function(b){0!=b.code?console.error(b.info):isNullOrUndefined(b.info)||isNullOrUndefined(a)||a(b.info)})};c.prototype.pointer_click=function(){this.setMode("pointer","pointer")};c.prototype.stock_click=function(){this.setMode("node","stock")};c.prototype.cloud_click=function(){this.setMode("node","cloud")};c.prototype.variable_click=function(){this.setMode("node",
"variable")};c.prototype.flow_click=function(){this.setMode("link","flow")};c.prototype.influence_click=function(){this.setMode("link","influence")};c.prototype.run_click=function(){var a=this;this.dosave(function(b){return a.doRunModel(b)})};c.prototype.doRunModel=function(a){$ts.post("@api:run/"+a,{guid:a},function(a){0!=a.code?console.error(a.info):$goto(a.url)})};return c}(Bootstrap);d.FlowEditor=b})(apps||(apps={}));
(function(d){function b(b){return b?b.toString().replace(".","_").replace("-","_").replace("+","_").replace("*","_").replace("~","_"):null}var f=/[-]?\d+/ig;d.ModelPatch=function(c){for(var a=0,d=c.nodeDataArray;a<d.length;a++){var e=d[a];e.isGroup||(Strings.IsPattern(e.key.toString(),f)&&(e.key="T"+e.key),e.key=b(e.key))}a=0;for(d=c.linkDataArray;a<d.length;a++)if(e=d[a],Strings.IsPattern(e.from.toString(),f)&&(e.from="T"+e.from),Strings.IsPattern(e.to.toString(),f)&&(e.to="T"+e.to),e.from=b(e.from),
e.to=b(e.to),!isNullOrEmpty(e.labelKeys))for(var h=0;h<e.labelKeys.length;h++)Strings.IsPattern(e.labelKeys[h].toString(),f)&&(e.labelKeys[h]="T"+e.labelKeys[h]),e.labelKeys[h]=b(e.labelKeys[h]);return c}})(apps||(apps={}));
(function(d){(function(b){b.nodeStyle=function(){return[{type:go.Panel.Spot,layerName:"Background",locationObjectName:"SHAPE",selectionObjectName:"SHAPE",locationSpot:go.Spot.Center},(new go.Binding("location","loc",go.Point.parse)).makeTwoWay(go.Point.stringify)]};b.shapeStyle=function(){return{name:"SHAPE",stroke:"black",fill:"#f0f0f0",portId:"",fromLinkable:!0,toLinkable:!0}};b.textStyle=function(){return[{font:"bold 11pt helvetica, bold arial, sans-serif",margin:2,editable:!0},(new go.Binding("text",
"label")).makeTwoWay()]}})(d.EditorTemplates||(d.EditorTemplates={}))})(apps||(apps={}));
(function(d){var b=function(b){function c(){var a=null!==b&&b.apply(this,arguments)||this;a.assemblyKey="ko00001-assembly";a.canvas=new d.Metabolic_pathway;return a}__extends(c,b);Object.defineProperty(c.prototype,"appName",{get:function(){return"Pathway_explorer"},enumerable:!0,configurable:!0});c.prototype.init=function(){var a=$ts("@data:repository"),b=this,c=localStorage.getItem(this.assemblyKey);Strings.Empty(a,!0)||(Strings.Empty(c)?$ts.get(a,function(a){b.saveCache(a);b.loadCache()}):b.loadCache());
b.canvas.init()};c.prototype.loadUITree=function(a){a=PathwayNavigator.parseJsTree(a);var b=$ts("@app:explorer");$("#"+b).jstree({core:{data:a.children},plugins:["contextmenu"],contextmenu:{items:{add_reactor:{label:"Add Reactor",action:c.addReactor}}}})};c.addReactor=function(a){a.reference[0].id.match(/K\d{4,}/g)};c.prototype.saveCache=function(a){var b=[],c=0;for(a=a.children;c<a.length;c++){var d=a[c],f=d.name.toLowerCase().replace(/\s+/ig,"_");b.push(f);localStorage.setItem(f,JSON.stringify(d))}localStorage.setItem(this.assemblyKey,
JSON.stringify(b))};c.prototype.loadCache=function(){var a=localStorage.getItem(this.assemblyKey),b={name:"ko00001",children:[]},c=0;for(a=JSON.parse(a);c<a.length;c++){var d=a[c];TypeScript.logging.log("load cache: "+d+"...");d=localStorage.getItem(d);b.children.push(JSON.parse(d))}this.loadUITree(b)};return c}(Bootstrap);d.PathwayExplorer=b})(apps||(apps={}));var PathwayNavigator;
(function(d){function b(c){var a=KEGG.brite.parseIDEntry(c.name),d=a.commonName||"KEGG";return isNullOrEmpty(c.children)?{id:a.id+"_"+ ++f,text:c.name,children:null,icon:"data:image/png;base64,"}:
{id:a.id+"_"+ ++f,text:d,children:$from(c.children).Select(b).ToArray(),icon:"data:image/png;base64,"}}
var f=1;d.parseJsTree=b})(PathwayNavigator||(PathwayNavigator={}));
var Graph=function(){function d(b){void 0===b&&(b=null);this.last_index=0;isNullOrUndefined(b)||(this.nodes=b.nodes,this.links=b.links)}d.prototype.objectify=function(){for(var b=this,d=b.links,c=[],a=function(a,e){var f=d[a];c.push(function(){var a;var c=b.nodes;var d=[];var e=0;for(a=c.length;e<a;e++){var g=c[e];f.source===g.id?f.source=g:f.target===g.id?f.target=g:d.push(void 0)}return d}())},g=0,e=d.length;g<e;g++)a(g,e);return c};d.prototype.remove=function(b){if(0<=__indexOf.call(this.nodes,
b))return this.nodes=this.nodes.filter(function(d){return d!==b}),this.links=this.links.filter(function(d){return d.source.id!==b.id&&d.target.id!==b.id});if(0<=__indexOf.call(this.links,b))return this.links=this.links.filter(function(d){return d!==b})};d.prototype.add_node=function(b){b={dunnartid:(this.last_index++).toString(),x:0,y:0,type:b};this.nodes.push(b);return b};d.prototype.add_link=function(b,d){if(b===d)return null;for(var c=this.links,a=0,f=c.length;a<f;a++){var e=c[a];if(e.source===
b&&e.target===d)return null}b={source:b,target:d};this.links.push(b);return b};return d}(),dataAdapter;
(function(d){var b=function(){function b(b){void 0===b&&(b="#embeddedsvg");this.nodeLookup={};this.sbsvg=d3.select($ts(b)).select("svg");this.graph={nodes:[],links:[],constraints:[],groups:[]}}b.prototype.getGraph=function(){var b=this;this.loadNodes(this.sbsvg);for(var a=0,d=this.sbsvg.selectAll(".connector")[0];a<d.length;a++){var e=d[a],f=e.getAttribute("dunnart:srcid");e=e.getAttribute("dunnart:dstid");f in this.nodeLookup?e in this.nodeLookup?this.graph.links.push({source:this.nodeLookup[f].index,
target:this.nodeLookup[e].index}):console.warn("missing node of "+e):console.warn("missing node of "+f)}a=0;for(d=this.sbsvg.selectAll(".cluster")[0];a<d.length;a++)f=d[a],this.graph.groups.push({leaves:f.getAttribute("dunnart:contains").split(" ").map(function(a){return b.nodeLookup[a].index}),style:f.getAttribute("style"),padding:10});a={};d=0;for(f=this.sbsvg.selectAll("[relType=alignment]")[0];d<f.length;d++){var k=f[d];e=k.getAttribute("constraintid");var l=this.nodeLookup[k.getAttribute("objoneid")].index;
k=parseInt(k.getAttribute("alignmentpos"));l={node:l,offset:0};e in a?a[e].offsets.push(l):this.graph.constraints.push(a[e]={type:"alignment",offsets:[l],axis:1==k?"y":"x"})}return this.graph};b.prototype.loadNodes=function(b){var a=0,c=0;for(b=b.selectAll("rect.shape")[0];c<b.length;c++){var d=b[c];this.graph.nodes.push(this.nodeLookup[d.id]={label:d.getAttribute("dunnart:label"),dunnartid:d.id,index:a++,width:60,height:40,x:parseFloat(d.getAttribute("x")),y:parseFloat(d.getAttribute("y")),rx:"rx"in
d.attributes?parseFloat(d.getAttribute("rx")):5,ry:"ry"in d.attributes?parseFloat(d.getAttribute("ry")):5})}};return b}();d.parseDunnart=b})(dataAdapter||(dataAdapter={}));
