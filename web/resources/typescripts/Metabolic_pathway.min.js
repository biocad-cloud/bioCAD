...
0;for(h=d.nodeDataArray;f<h.length;f++)e=h[f],!isNullOrUndefined(e.isGroup)&&e.isGroup||"valve"==e.category||(c.nodes.push({dunnartid:(c.nodes.length+1).toString(),height:40,index:c.nodes.length,label:e.label,rx:9,ry:9,width:60,x:0,y:0}),a[e.key]=c.nodes.length-1,isNullOrUndefined(e.group)||g[e.group.toString()].leaves.push(c.nodes.length-1));e=0;for(d=d.linkDataArray;e<d.length;e++)f=d[e],c.links.push({source:a[f.from],target:a[f.to]});for(var m in g)c.groups.push(g[m]);return c}})(b.translation||
(b.translation={}))})(apps||(apps={}));
...
"flow"===a.itemType?go.Link.Normal:go.Link.Bezier;this.temporaryLink.path.stroke="flow"===a.itemType?"blue":"green";this.temporaryLink.path.strokeWidth="flow"===a.itemType?5:1;go.LinkingTool.prototype.doActivate.call(this)},"linkingTool.insertLink":function(b,d,g,e){c.myDiagram.model.setCategoryForLinkData(this.archetypeLinkData,a.itemType);this.archetypeLabelNodeData="flow"===a.itemType?{category:"valve"}:null;this.archetypeLinkData.text=a.itemType;return go.LinkingTool.prototype.insertLink.call(this,
