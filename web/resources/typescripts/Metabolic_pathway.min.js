...
function(a){return c.d3cola.on("tick",c.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};b.prototype.loadGraph=function(c){var a=this,g=this.vis.append("g"),f=this.vis.append("g"),e=this.vis.append("g");console.log(JSON.stringify(c));this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",
...
(b.allowLink=!1,b.nodes.each(function(a){return a.port.cursor=""})):"link"===a&&(b.allowLink=!0,b.nodes.each(function(a){return a.port.cursor="pointer"}));b.commitTransaction("mode changed")};c.prototype.save_click=function(){this.dosave()};c.prototype.load=function(){var a=this;$ts.getText("@api:load?model_id="+$ts("@data:model_id"),function(b){a.myDiagram.model=go.Model.fromJson(b)})};c.prototype.dosave=function(a){void 0===a&&(a=null);var b=this.myDiagram,c=b.model.toJson();c={guid:$ts("@data:model_id"),
model:d.ModelPatch(JSON.parse(c)),type:"dynamics"};b.isModified=!1;$ts.post("@api:save",c,function(b){0!=b.code?console.error(b.info):isNullOrUndefined(b.info)||isNullOrUndefined(a)||a(b.info)})};c.prototype.pointer_click=function(){this.setMode("pointer","pointer")};c.prototype.stock_click=function(){this.setMode("node","stock")};c.prototype.cloud_click=function(){this.setMode("node","cloud")};c.prototype.variable_click=function(){this.setMode("node","variable")};c.prototype.flow_click=function(){this.setMode("link",
"flow")};c.prototype.influence_click=function(){this.setMode("link","influence")};c.prototype.run_click=function(){var a=this;this.dosave(function(b){return a.doRunModel(b)})};c.prototype.doRunModel=function(a){$ts.post("@api:run/"+a,{guid:a},function(a){0!=a.code?console.error(a.info):$goto(a.url)})};return c}(Bootstrap);d.FlowEditor=b})(apps||(apps={}));
(function(d){function b(b){return b.replace(".","_").replace("-","_").replace("+","_").replace("*","_").replace("~","_")}var e=/[-]?\d+/ig;d.ModelPatch=function(c){for(var a=0;a<c.nodeDataArray.length;a++){var d=c.nodeDataArray[a];Strings.IsPattern(d.key.toString(),e)&&(d.key=b("T"+d.key));c.nodeDataArray[a]=d}for(d=0;d<c.linkDataArray.length;d++){var f=c.linkDataArray[d];Strings.IsPattern(f.from.toString(),e)&&(f.from=b("T"+f.from));Strings.IsPattern(f.to.toString(),e)&&(f.to=b("T"+f.to));if(!isNullOrEmpty(f.labelKeys))for(a=
0;a<f.labelKeys.length;a++)Strings.IsPattern(f.labelKeys[a].toString(),e)&&(f.labelKeys[a]=b("T"+f.labelKeys[a]));c.linkDataArray[d]=f}return c}})(apps||(apps={}));
