...
var __extends=this&&this.__extends||function(){var b=function(d,e){b=Object.setPrototypeOf||{__proto__:[]}instanceof Array&&function(c,a){c.__proto__=a}||function(c,a){for(var h in a)a.hasOwnProperty(h)&&(c[h]=a[h])};return b(d,e)};return function(d,e){function c(){this.constructor=d}b(d,e);d.prototype=null===e?Object.create(e):(c.prototype=e.prototype,new c)}}(),biodeep;
...
function(c){var a=this,h=this.vis.append("g"),f=this.vis.append("g"),g=this.vis.append("g");Strings.Empty(c.class)||"GraphLinksModel"!=c.class||(c=b.translation.translateToColaGraph(c));console.log(JSON.stringify(c));console.log("view nodes details:");console.table(c.nodes);this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",5).attr("markerWidth",
3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");h=h.selectAll(".group").data(c.groups).enter().append("rect").attr("rx",8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});g=g.selectAll(".link").data(c.links).enter().append("line").attr("class","link");var e=f.selectAll(".node").data(c.nodes).enter().append("rect").attr("class","node").attr("width",function(c){return c.width+
2*a.pad+2*a.margin}).attr("height",function(c){return c.height+2*a.pad+2*a.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);c=f.selectAll(".label").data(c.nodes).enter().append("text").attr("class","label").call(this.d3cola.drag);c.each(d.insertLinebreaks);e.append("title").text(function(a){return a.label});return new b.Cola_graph(e,g,h,c,this)};d.insertLinebreaks=function(c){var a=d3.select(this);c=c.label.split(" ");a.text("");for(var b=0;b<
...
(function(b){(function(b){b.paper_colors="#D02823 #0491d0 #88bb64 #15DBFF #583B73 #f2ce3f #8858BF #CCFF33 #fb5b44 #361f32 #DF2789 #396b1c".split(" ");b.translateToColaGraph=function(d){for(var c={groups:[],constraints:[],links:[],nodes:[]},a={},h={},f=0,g=d.nodeDataArray;f<g.length;f++){var e=g[f];if(!isNullOrUndefined(e.isGroup)&&e.isGroup){var k=b.paper_colors[Object.keys(h).length];h[e.key.toString()]={leaves:[],padding:10,style:"fill:"+k+";fill-opacity:0.31764700000000001;stroke:"+k+";stroke-opacity:1"}}}f=
0;for(g=d.nodeDataArray;f<g.length;f++)e=g[f],!isNullOrUndefined(e.isGroup)&&e.isGroup||"valve"==e.category||(k=e.loc,k=Strings.Empty(k)?[0,0]:k.split(/\s+/ig).map(function(a){return parseFloat(a)}),2>k.length&&(k=[0,0]),c.nodes.push({dunnartid:(c.nodes.length+1).toString(),height:40,index:c.nodes.length,label:e.label,rx:9,ry:9,width:60,x:k[0],y:k[1]}),a[e.key]=c.nodes.length-1,isNullOrUndefined(e.group)||h[e.group.toString()].leaves.push(c.nodes.length-1));e=0;for(d=d.linkDataArray;e<d.length;e++)f=
d[e],c.links.push({source:a[f.from],target:a[f.to]});for(var m in h)c.groups.push(h[m]);return c}})(b.translation||(b.translation={}))})(apps||(apps={}));
...
f.to=d(f.to),!isNullOrEmpty(f.labelKeys))for(var g=0;g<f.labelKeys.length;g++)Strings.IsPattern(f.labelKeys[g].toString(),e)&&(f.labelKeys[g]="T"+f.labelKeys[g]),f.labelKeys[g]=d(f.labelKeys[g]);return c}})(apps||(apps={}));
...
(function(b){b.listDiv="#sample_suggests";b.inputDiv="#sample_search";var d=function(d){function c(){return null!==d&&d.apply(this,arguments)||this}__extends(c,d);Object.defineProperty(c.prototype,"appName",{get:function(){return"kegg_network"},enumerable:!0,configurable:!0});c.prototype.init=function(){var a=this,d=PathwayNavigator.parseJsTree(b.PathwayExplorer.loadKEGGTree()),e=[];c.createSet(d,e);d=[];for(var g=0,l=$from(e).GroupBy(function(a){return a.text.split(/\s+/ig)[0]}).Select(function(a){return a.First}).ToArray();g<l.length;g++){var k=
l[g];d.push(new Application.Suggestion.term(k.id,k.text))}var m=Application.Suggestion.render.makeSuggestions(d,b.listDiv,function(c){return a.clickOnTerm(c)},13,!0,"");$ts(b.inputDiv).onkeyup=function(){var a=$ts.value(b.inputDiv);Strings.Empty(a,!0)?$ts(b.listDiv).hide():($ts(b.listDiv).show(),m(a))};TypeScript.logging.log(e.length+" kegg components has been loaded!",TypeScript.ConsoleColors.Magenta);TypeScript.logging.log(" ~done!",TypeScript.ConsoleColors.Magenta)};c.prototype.clickOnTerm=function(a){$ts(b.listDiv).hide()};
c.createSet=function(a,c){var b=0;for(a=a.children;b<a.length;b++){var d=a[b];isNullOrUndefined(d.children)?c.push(d):this.createSet(d,c)}};return c}(Bootstrap);b.KEGGNetwork=d})(apps||(apps={}));
var Graph=function(){function b(b){void 0===b&&(b=null);this.last_index=0;isNullOrUndefined(b)||(this.nodes=b.nodes,this.links=b.links)}b.prototype.objectify=function(){for(var b=this,e=b.links,c=[],a=function(a,d){var f=e[a];c.push(function(){var a;var c=b.nodes;var d=[];var e=0;for(a=c.length;e<a;e++){var h=c[e];f.source===h.id?f.source=h:f.target===h.id?f.target=h:d.push(void 0)}return d}())},h=0,f=e.length;h<f;h++)a(h,f);return c};b.prototype.remove=function(b){if(0<=__indexOf.call(this.nodes,
...
(function(b){var d=function(){function b(b){void 0===b&&(b="#embeddedsvg");this.nodeLookup={};this.sbsvg=d3.select($ts(b)).select("svg");this.graph={nodes:[],links:[],constraints:[],groups:[]}}b.prototype.getGraph=function(){var b=this;this.loadNodes(this.sbsvg);for(var a=0,d=this.sbsvg.selectAll(".connector")[0];a<d.length;a++){var e=d[a],g=e.getAttribute("dunnart:srcid");e=e.getAttribute("dunnart:dstid");g in this.nodeLookup?e in this.nodeLookup?this.graph.links.push({source:this.nodeLookup[g].index,
target:this.nodeLookup[e].index}):console.warn("missing node of "+e):console.warn("missing node of "+g)}a=0;for(d=this.sbsvg.selectAll(".cluster")[0];a<d.length;a++)g=d[a],this.graph.groups.push({leaves:g.getAttribute("dunnart:contains").split(" ").map(function(a){return b.nodeLookup[a].index}),style:g.getAttribute("style"),padding:10});a={};d=0;for(g=this.sbsvg.selectAll("[relType=alignment]")[0];d<g.length;d++){var l=g[d];e=l.getAttribute("constraintid");var k=this.nodeLookup[l.getAttribute("objoneid")].index;
l=parseInt(l.getAttribute("alignmentpos"));k={node:k,offset:0};e in a?a[e].offsets.push(k):this.graph.constraints.push(a[e]={type:"alignment",offsets:[k],axis:1==l?"y":"x"})}return this.graph};b.prototype.loadNodes=function(b){var a=0,c=0;for(b=b.selectAll("rect.shape")[0];c<b.length;c++){var d=b[c];this.graph.nodes.push(this.nodeLookup[d.id]={label:d.getAttribute("dunnart:label"),dunnartid:d.id,index:a++,width:60,height:40,x:parseFloat(d.getAttribute("x")),y:parseFloat(d.getAttribute("y")),rx:"rx"in
