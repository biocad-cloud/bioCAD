...
(function(b){(function(b){b.paper_colors="#D02823 #0491d0 #88bb64 #15DBFF #583B73 #f2ce3f #8858BF #CCFF33 #fb5b44 #361f32 #DF2789 #396b1c".split(" ");b.translateToColaGraph=function(d){for(var c={groups:[],constraints:[],links:[],nodes:[]},a={},g={},f=0,e=d.nodeDataArray;f<e.length;f++){var h=e[f];if(!isNullOrUndefined(h.isGroup)&&h.isGroup){var l=b.paper_colors[Object.keys(g).length];g[h.key.toString()]={leaves:[],padding:10,style:"fill:"+l+";fill-opacity:0.31764700000000001;stroke:"+l+";stroke-opacity:1"}}}f=
0;for(e=d.nodeDataArray;f<e.length;f++)h=e[f],!isNullOrUndefined(h.isGroup)&&h.isGroup||"valve"==h.category||(c.nodes.push({dunnartid:(c.nodes.length+1).toString(),height:40,index:c.nodes.length,label:h.label,rx:9,ry:9,width:60,x:0,y:0}),a[h.key]=c.nodes.length-1,isNullOrUndefined(h.group)||g[h.group.toString()].leaves.push(c.nodes.length-1));h=0;for(d=d.linkDataArray;h<d.length;h++)f=d[h],c.links.push({source:a[f.from],target:a[f.to]});for(var m in g)c.groups.push(g[m]);return c}})(b.translation||
...
target:this.nodeLookup[f].index}):console.warn("missing node of "+f):console.warn("missing node of "+e)}a=0;for(d=this.sbsvg.selectAll(".cluster")[0];a<d.length;a++)e=d[a],this.graph.groups.push({leaves:e.getAttribute("dunnart:contains").split(" ").map(function(a){return b.nodeLookup[a].index}),style:e.getAttribute("style"),padding:10});a={};d=0;for(e=this.sbsvg.selectAll("[relType=alignment]")[0];d<e.length;d++){var h=e[d];f=h.getAttribute("constraintid");var l=this.nodeLookup[h.getAttribute("objoneid")].index;
h=parseInt(h.getAttribute("alignmentpos"));l={node:l,offset:0};f in a?a[f].offsets.push(l):this.graph.constraints.push(a[f]={type:"alignment",offsets:[l],axis:1==h?"y":"x"})}return this.graph};b.prototype.loadNodes=function(b){var a=0,c=0;for(b=b.selectAll("rect.shape")[0];c<b.length;c++){var d=b[c];this.graph.nodes.push(this.nodeLookup[d.id]={label:d.getAttribute("dunnart:label"),dunnartid:d.id,index:a++,width:60,height:40,x:parseFloat(d.getAttribute("x")),y:parseFloat(d.getAttribute("y")),rx:"rx"in
