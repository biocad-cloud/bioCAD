...
$jscomp.polyfill=function(d,b,f,c){if(b){f=$jscomp.global;d=d.split(".");for(c=0;c<d.length-1;c++){var a=d[c];a in f||(f[a]={});f=f[a]}d=d[d.length-1];c=f[d];b=b(c);b!=c&&null!=b&&$jscomp.defineProperty(f,d,{configurable:!0,writable:!0,value:b})}};$jscomp.underscoreProtoCanBeSet=function(){var d={a:!0},b={};try{return b.__proto__=d,b.a}catch(f){}return!1};
...
function(a){return c.d3cola.on("tick",c.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};b.prototype.loadGraph=function(c){var a=this,g=this.vis.append("g"),e=this.vis.append("g"),h=this.vis.append("g");Strings.Empty(c.class)||"GraphLinksModel"!=c.class||(c=d.translation.translateToColaGraph(c));console.log(JSON.stringify(c));this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();
...
"link");var f=e.selectAll(".node").data(c.nodes).enter().append("rect").attr("class","node").attr("width",function(c){return c.width+2*a.pad+2*a.margin}).attr("height",function(c){return c.height+2*a.pad+2*a.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);c=e.selectAll(".label").data(c.nodes).enter().append("text").attr("class","label").call(this.d3cola.drag);c.each(b.insertLinebreaks);f.append("title").text(function(a){return a.label});return new d.Cola_graph(f,
h,g,c,this)};b.insertLinebreaks=function(c){var a=d3.select(this);c=c.label.split(" ");a.text("");for(var g=0;g<c.length;g++){var e=c[g];e=a.append("tspan").text(e);e.attr("x",0).attr("dy","15").attr("font-size","12")}};return b}();d.Metabolic_pathway=b})(apps||(apps={}));
(function(d){(function(b){b.translateToColaGraph=function(b){for(var c={groups:[],constraints:[],links:[],nodes:[]},a={},g=0,e=b.nodeDataArray;g<e.length;g++){var d=e[g];if(isNullOrUndefined(d.isGroup)||!d.isGroup)c.nodes.push({dunnartid:(c.nodes.length+1).toString(),height:40,index:c.nodes.length,label:d.label,rx:9,ry:9,width:60,x:0,y:0}),a[d.key]=c.nodes.length-1}g=0;for(b=b.linkDataArray;g<b.length;g++)e=b[g],c.links.push({source:a[e.from],target:a[e.to]});return c}})(d.translation||(d.translation=
{}))})(apps||(apps={}));
...
a.adornedPart;var c=a.memberParts.count,b=0;a.memberParts.each(function(a){a instanceof go.Link&&b++});return"Group "+a.data.key+": "+a.data.text+"\n"+c+" members including "+b+" links"};c.prototype.setMode=function(a,c){var b=this.myDiagram,g=this.SD;b.startTransaction();document.getElementById(g.itemType).className=g.mode+"_normal";document.getElementById(c).className=a+"_selected";g.mode=a;g.itemType=c;"pointer"===a?(b.allowLink=!1,b.nodes.each(function(a){return a.port.cursor=""})):"node"===a?
...
(function(d){function b(c){return c?c.toString().replace(".","_").replace("-","_").replace("+","_").replace("*","_").replace("~","_"):null}var f=/[-]?\d+/ig;d.ModelPatch=function(c){for(var a=0,d=c.nodeDataArray;a<d.length;a++){var e=d[a];e.isGroup||(Strings.IsPattern(e.key.toString(),f)&&(e.key="T"+e.key),e.key=b(e.key))}a=0;for(d=c.linkDataArray;a<d.length;a++)if(e=d[a],Strings.IsPattern(e.from.toString(),f)&&(e.from="T"+e.from),Strings.IsPattern(e.to.toString(),f)&&(e.to="T"+e.to),e.from=b(e.from),
e.to=b(e.to),!isNullOrEmpty(e.labelKeys))for(var h=0;h<e.labelKeys.length;h++)Strings.IsPattern(e.labelKeys[h].toString(),f)&&(e.labelKeys[h]="T"+e.labelKeys[h]),e.labelKeys[h]=b(e.labelKeys[h]);return c}})(apps||(apps={}));
