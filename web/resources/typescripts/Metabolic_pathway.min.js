var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.SIMPLE_FROUND_POLYFILL=!1;$jscomp.defineProperty=$jscomp.ASSUME_ES5||"function"==typeof Object.defineProperties?Object.defineProperty:function(e,d,c){e!=Array.prototype&&e!=Object.prototype&&(e[d]=c.value)};$jscomp.getGlobal=function(e){return"undefined"!=typeof window&&window===e?e:"undefined"!=typeof global&&null!=global?global:e};$jscomp.global=$jscomp.getGlobal(this);
$jscomp.polyfill=function(e,d,c,b){if(d){c=$jscomp.global;e=e.split(".");for(b=0;b<e.length-1;b++){var a=e[b];a in c||(c[a]={});c=c[a]}e=e[e.length-1];b=c[e];d=d(b);d!=b&&null!=d&&$jscomp.defineProperty(c,e,{configurable:!0,writable:!0,value:d})}};$jscomp.underscoreProtoCanBeSet=function(){var e={a:!0},d={};try{return d.__proto__=e,d.a}catch(c){}return!1};
$jscomp.setPrototypeOf="function"==typeof Object.setPrototypeOf?Object.setPrototypeOf:$jscomp.underscoreProtoCanBeSet()?function(e,d){e.__proto__=d;if(e.__proto__!==d)throw new TypeError(e+" is not extensible");return e}:null;$jscomp.polyfill("Object.setPrototypeOf",function(e){return e||$jscomp.setPrototypeOf},"es6","es5");
var __extends=this&&this.__extends||function(){var e=function(d,c){e=Object.setPrototypeOf||{__proto__:[]}instanceof Array&&function(b,a){b.__proto__=a}||function(b,a){for(var f in a)a.hasOwnProperty(f)&&(b[f]=a[f])};return e(d,c)};return function(d,c){function b(){this.constructor=d}e(d,c);d.prototype=null===c?Object.create(c):(b.prototype=c.prototype,new b)}}(),biodeep;
(function(e){(function(d){d.start=function(){Router.AddAppHandler(new apps.PathwayExplorer);Router.RunApp()}})(e.app||(e.app={}))})(biodeep||(biodeep={}));var __indexOf=Array.prototype.indexOf||function(e){for(var d=0,c=this.length;d<c;d++)if(d in this&&this[d]===e)return d;return-1};$ts.mode=Modes.debug;$ts(biodeep.app.start);var apps;
(function(e){var d=function(){function c(b,a,f,c,d){this.node=b;this.link=a;this.group=f;this.label=c;this.vm=d;console.log(this)}c.prototype.tick=function(){var b=this.vm,a=this;return function(){a.node.each(function(a){return a.innerBounds=a.bounds.inflate(-b.margin)});a.link.each(function(a){return a.route=cola.makeEdgeBetween(a.source.innerBounds,a.target.innerBounds,5)});a.link.attr("x1",function(a){return a.route.sourceIntersection.x}).attr("y1",function(a){return a.route.sourceIntersection.y}).attr("x2",
function(a){return a.route.arrowStart.x}).attr("y2",function(a){return a.route.arrowStart.y});a.label.each(function(a){var f=this.getBBox();a.width=f.width+2*b.margin+8;a.height=f.height+2*b.margin+8});a.node.attr("x",function(a){return a.innerBounds.x}).attr("y",function(a){return a.innerBounds.y}).attr("width",function(a){return a.innerBounds.width()}).attr("height",function(a){return a.innerBounds.height()});a.group.attr("x",function(a){return a.bounds.x}).attr("y",function(a){return a.bounds.y}).attr("width",
function(a){return a.bounds.width()}).attr("height",function(a){return a.bounds.height()});a.label.attr("transform",function(b){return a.transform(b)})}};c.prototype.transform=function(b){return"translate("+b.x+this.vm.margin+", "+(b.y+this.vm.margin-b.height/2)+")"};return c}();e.Cola_graph=d})(apps||(apps={}));
(function(e){var d=function(){function c(){this.width=800;this.height=520;this.margin=6;this.pad=12;this.width=parseFloat($ts("@width"));this.height=parseFloat($ts("@height"))}c.prototype.redraw=function(){this.vis.attr("transform","translate("+d3.event.translate+") scale("+d3.event.scale+")")};c.prototype.savePng=function(){saveSvgAsPng($ts("#canvas").childNodes.item(0),"pathway.png")};c.prototype.init=function(){var b=this,a=$ts("@url:graph");this.d3cola=cola.d3adaptor(d3).linkDistance(60).avoidOverlaps(!0).size([this.width,
this.height]);this.outer=d3.select($ts("#canvas")).append("svg").attr("width",this.width).attr("height",this.height).attr("pointer-events","all");this.outer.append("rect").attr("class","background").attr("width","100%").attr("height","100%").call(d3.behavior.zoom().on("zoom",function(){return b.redraw()}));this.vis=this.outer.append("g").attr("transform","translate(80,80) scale(0.7)");if("#"==a.charAt(0))this.d3cola.on("tick",this.loadGraph((new dataAdapter.parseDunnart(a)).getGraph()).tick());else $ts.getText(a,
function(a){return b.d3cola.on("tick",b.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};c.prototype.loadGraph=function(b){var a=this,f=this.vis.append("g"),d=this.vis.append("g"),l=this.vis.append("g");console.log(JSON.stringify(b));this.d3cola.nodes(b.nodes).links(b.links).groups(b.groups).constraints(b.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",
5).attr("markerWidth",3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");f=f.selectAll(".group").data(b.groups).enter().append("rect").attr("rx",8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});l=l.selectAll(".link").data(b.links).enter().append("line").attr("class","link");var h=d.selectAll(".node").data(b.nodes).enter().append("rect").attr("class","node").attr("width",function(b){return b.width+
2*a.pad+2*a.margin}).attr("height",function(b){return b.height+2*a.pad+2*a.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);b=d.selectAll(".label").data(b.nodes).enter().append("text").attr("class","label").call(this.d3cola.drag);b.each(c.insertLinebreaks);h.append("title").text(function(a){return a.label});return new e.Cola_graph(h,l,f,b,this)};c.insertLinebreaks=function(b){var a=d3.select(this);b=b.label.split(" ");a.text("");for(var f=0;f<
b.length;f++){var c=b[f];c=a.append("tspan").text(c);c.attr("x",0).attr("dy","15").attr("font-size","12")}};return c}();e.Metabolic_pathway=d})(apps||(apps={}));
(function(e){var d=function(){function c(){this.width=960;this.height=500;this.drag_link=this.drag=this.new_link_source=this.force=this.colorify=this.vis=this.selection=null}c.prototype.tick=function(){this.vis.selectAll(".node").attr("transform",function(b){return"translate("+b.x+","+b.y+")"});return this.vis.selectAll(".link").attr("x1",function(b){return b.source.x}).attr("y1",function(b){return b.source.y}).attr("x2",function(b){return b.target.x}).attr("y2",function(b){return b.target.y})};c.prototype.click=
function(b){this.selection=null;d3.selectAll(".node").classed("selected",!1);return d3.selectAll(".link").classed("selected",!1)};c.prototype.zoom=function(){return this.vis.attr("transform","translate("+d3.event.translate+")scale("+d3.event.scale+")")};c.prototype.init=function(){var b=this,a=d3.select("body").append("svg").attr("width",this.width).attr("height",this.height).append("g"),f=this;this.graph.objectify();a.call(d3.behavior.zoom().scaleExtent([.5,8]).on("zoom",function(){return f.zoom()}));
f.vis=a.append("g");f.vis.append("rect").attr("class","overlay").attr("x",-5E5).attr("y",-5E5).attr("width",1E6).attr("height",1E6).on("click",function(a){return b.click(a)});f.colorify=d3.scale.category10();f.force=d3.layout.force().size([this.width,this.height]).charge(-400).linkDistance(60).on("tick",function(){return f.tick()});f.drag=f.force.drag().on("dragstart",function(a){return a.fixed=!0});d3.select(window).on("keydown",function(){if(46===d3.event.keyCode&&null!=f.selection)return f.graph.remove(f.selection),
f.selection=null,f.update()});this.update();this.toolbar()};c.prototype.toolbar=function(){var b=$("<div class='toolbar'></div>"),a=this,f=$("<div class='library'></div></div>");$("body").append(b);b.append($(e.pointer));b.append($(e.add_node));b.append($(e.add_link));b.append(f);["X","Y","Z","W"].forEach(function(b){var c=$(e.buttonHtml(a,b));c.bind("click",function(){a.graph.add_node(b);return a.update()});f.append(c);return f.hide()});a.tool="pointer";a.new_link_source=null;a.vis.on("mousemove.add_link",
function(b){if(null!=a.new_link_source)return b=d3.mouse(a.vis.node()),a.drag_link.attr("x1",a.new_link_source.x).attr("y1",a.new_link_source.y).attr("x2",b[0]).attr("y2",b[1])}).on("mouseup.add_link",function(b){a.new_link_source=null;if(null!=a.drag_link)return a.drag_link.remove()});return d3.selectAll(".tool").on("click",function(){return a.tool_click(f)})};c.prototype.tool_click=function(b){d3.selectAll(".tool").classed("active",!1);d3.select(this).classed("active",!0);var a=$(this).data("tool"),
c=this.vis.selectAll(".node");"add_link"===a&&"add_link"!==this.tool?(c.on("mousedown.drag",null).on("touchstart.drag",null),c.call(this.drag_add_link)):"add_link"!==a&&"add_link"===this.tool&&(c.on("mousedown.add_link",null).on("mouseup.add_link",null),c.call(this.drag));"add_node"===a?b.show():b.hide();return this.tool=a};c.prototype.node_click=function(b){this.selection=b;d3.selectAll(".node").classed("selected",function(a){return a===b});return d3.selectAll(".link").classed("selected",!1)};c.prototype.link_click=
function(b){this.selection=b;d3.selectAll(".link").classed("selected",function(a){return a===b});return d3.selectAll(".node").classed("selected",!1)};c.prototype.update=function(){var b=this.graph,a=this;a.force.nodes(b.nodes).links(b.links).start();var c=a.vis.selectAll(".node").data(b.nodes,function(a){return a.id}),d=c.enter().append("g").attr("class","node").on("click",function(b){return a.node_click(b)});b=a.vis.selectAll(".link").data(b.links,function(a){return""+a.source.id+"->"+a.target.id});
b.enter().insert("line",".node").attr("class","link").on("click",function(b){return a.link_click(b)});b.exit().remove();"add_link"===a.tool?d.call(this.drag_add_link):d.call(a.drag);d.append("circle").attr("r",18).attr("stroke",function(b){return a.colorify(b.type)}).attr("fill",function(b){return d3.hcl(a.colorify(b.type)).brighter(3)});d.append("text").text(function(a){return a.id}).attr("dy","0.35em").attr("fill",function(b){return a.colorify(b.type)});return c.exit().remove()};c.prototype.drag_add_link=
function(b){var a=this,c=this.graph;return b.on("mousedown.add_link",function(b){a.new_link_source=b;var c=d3.mouse(a.vis.node());a.drag_link=a.vis.insert("line",".node").attr("class","drag_link").attr("x1",b.x).attr("y1",b.y).attr("x2",c[0]).attr("y2",c[1]);d3.event.stopPropagation();return d3.event.preventDefault()}).on("mouseup.add_link",function(b){if(null!=c.add_link(a.new_link_source,b))return a.update()})};return c}();e.GraphEditor=d})(apps||(apps={}));
(function(e){e.buttonHtml=function(d,c){return"\n            <svg width='42' height='42'>\n                <g class='node'>\n                    <circle cx = '21' cy = '21' r = '18'\n                        stroke = '"+d.colorify(c)+"'\n                        fill = '"+d3.hcl(d.colorify(c)).brighter(3)+"'\n                    />\n                </g>\n            </svg>\n        "};e.add_link="\n        <svg class='tool' data-tool='add_link' xmlns='http://www.w3.org/2000/svg' version='1.1' width='32' height='32' viewBox='0 0 128 128'>\n            <g transform='translate(557.53125,-356.22543)'>\n                <g transform='translate(20,0)'>\n                    <path d='m -480.84375,360 c -15.02602,0 -27.375,12.31773 -27.375,27.34375 0,4.24084 1.00221,8.28018 2.75,11.875 l -28.875,28.875 c -3.59505,-1.74807 -7.6338,-2.75 -11.875,-2.75 -15.02602,0 -27.34375,12.34898 -27.34375,27.375 0,15.02602 12.31773,27.34375 27.34375,27.34375 15.02602,0 27.375,-12.31773 27.375,-27.34375 0,-4.26067 -0.98685,-8.29868 -2.75,-11.90625 L -492.75,411.96875 c 3.60156,1.75589 7.65494,2.75 11.90625,2.75 15.02602,0 27.34375,-12.34898 27.34375,-27.375 C -453.5,372.31773 -465.81773,360 -480.84375,360 z m 0,14 c 7.45986,0 13.34375,5.88389 13.34375,13.34375 0,7.45986 -5.88389,13.375 -13.34375,13.375 -7.45986,0 -13.375,-5.91514 -13.375,-13.375 0,-7.45986 5.91514,-13.34375 13.375,-13.34375 z m -65.375,65.34375 c 7.45986,0 13.34375,5.91514 13.34375,13.375 0,7.45986 -5.88389,13.34375 -13.34375,13.34375 -7.45986,0 -13.34375,-5.88389 -13.34375,-13.34375 0,-7.45986 5.88389,-13.375 13.34375,-13.375 z'/>\n                    <path d='m -484.34375,429.25 c -1.95543,0.19978 -3.60373,2.03442 -3.59375,4 l 0,12.40625 -12.40625,0 c -2.09434,2.1e-4 -3.99979,1.90566 -4,4 l 0,10 c -0.007,0.1353 -0.007,0.27095 0,0.40625 0.19978,1.95543 2.03442,3.60373 4,3.59375 l 12.40625,0 0,12.4375 c 2.1e-4,2.09434 1.90566,3.99979 4,4 l 10,0 c 2.09434,-2.1e-4 3.99979,-1.90566 4,-4 l 0,-12.4375 12.4375,0 c 2.09434,-2.1e-4 3.99979,-1.90566 4,-4 l 0,-10 c -2.1e-4,-2.09434 -1.90566,-3.99979 -4,-4 l -12.4375,0 0,-12.40625 c -2.1e-4,-2.09434 -1.90566,-3.99979 -4,-4 l -10,0 c -0.1353,-0.007 -0.27095,-0.007 -0.40625,0 z'/>\n                </g>\n            </g>\n        </svg>\n    ";
e.add_node="\n        <svg class='tool' data-tool='add_node' xmlns='http://www.w3.org/2000/svg' version='1.1' width='32' height='32' viewBox='0 0 128 128'>\n            <g transform='translate(720.71649,-356.22543)'>\n                <g transform='translate(-3.8571429,146.42857)'>\n                    <path d='m -658.27638,248.37149 c -1.95543,0.19978 -3.60373,2.03442 -3.59375,4 l 0,12.40625 -12.40625,0 c -2.09434,2.1e-4 -3.99979,1.90566 -4,4 l 0,10 c -0.007,0.1353 -0.007,0.27095 0,0.40625 0.19978,1.95543 2.03442,3.60373 4,3.59375 l 12.40625,0 0,12.4375 c 2.1e-4,2.09434 1.90566,3.99979 4,4 l 10,0 c 2.09434,-2.1e-4 3.99979,-1.90566 4,-4 l 0,-12.4375 12.4375,0 c 2.09434,-2.1e-4 3.99979,-1.90566 4,-4 l 0,-10 c -2.1e-4,-2.09434 -1.90566,-3.99979 -4,-4 l -12.4375,0 0,-12.40625 c -2.1e-4,-2.09434 -1.90566,-3.99979 -4,-4 l -10,0 c -0.1353,-0.007 -0.27095,-0.007 -0.40625,0 z'/>\n                    <path d='m -652.84375,213.9375 c -32.97528,0 -59.875,26.86847 -59.875,59.84375 0,32.97528 26.89972,59.875 59.875,59.875 32.97528,0 59.84375,-26.89972 59.84375,-59.875 0,-32.97528 -26.86847,-59.84375 -59.84375,-59.84375 z m 0,14 c 25.40911,0 45.84375,20.43464 45.84375,45.84375 0,25.40911 -20.43464,45.875 -45.84375,45.875 -25.40911,0 -45.875,-20.46589 -45.875,-45.875 0,-25.40911 20.46589,-45.84375 45.875,-45.84375 z'/>\n                </g>\n            </g>\n        </svg>\n    ";
e.pointer="\n        <svg class='active tool' data-tool='pointer' xmlns='http://www.w3.org/2000/svg' version='1.1' width='32' height='32' viewBox='0 0 128 128'>\n            <g transform='translate(881.10358,-356.22543)'>\n                <g transform='matrix(0.8660254,-0.5,0.5,0.8660254,-266.51112,-215.31898)'>\n                    <path d='m -797.14902,212.29589 a 5.6610848,8.6573169 0 0 0 -4.61823,4.3125 l -28.3428,75.0625 a 5.6610848,8.6573169 0 0 0 4.90431,13 l 56.68561,0 a 5.6610848,8.6573169 0 0 0 4.9043,-13 l -28.3428,-75.0625 a 5.6610848,8.6573169 0 0 0 -5.19039,-4.3125 z m 0.28608,25.96875 18.53419,49.09375 -37.06838,0 18.53419,-49.09375 z'/>\n                    <path d='m -801.84375,290.40625 c -2.09434,2.1e-4 -3.99979,1.90566 -4,4 l 0,35.25 c 2.1e-4,2.09434 1.90566,3.99979 4,4 l 10,0 c 2.09434,-2.1e-4 3.99979,-1.90566 4,-4 l 0,-35.25 c -2.1e-4,-2.09434 -1.90566,-3.99979 -4,-4 z'/>\n                </g>\n            </g>\n        </svg>\n    "})(apps||
(apps={}));
(function(e){var d=function(c){function b(){var a=null!==c&&c.apply(this,arguments)||this;a.assemblyKey="ko00001-assembly";a.canvas=new e.Metabolic_pathway;return a}__extends(b,c);Object.defineProperty(b.prototype,"appName",{get:function(){return"Pathway_explorer"},enumerable:!0,configurable:!0});b.prototype.init=function(){var a=$ts("@data:repository"),b=this,c=localStorage.getItem(this.assemblyKey);Strings.Empty(c)?$ts.get(a,function(a){b.saveCache(a);b.loadCache()}):b.loadCache();b.canvas.init()};b.prototype.loadUITree=
function(a){a=PathwayNavigator.parseJsTree(a);var c=$ts("@app:explorer");$("#"+c).jstree({core:{data:a.children},plugins:["contextmenu"],contextmenu:{items:{add_reactor:{label:"Add Reactor",action:b.addReactor}}}})};b.addReactor=function(a){a.reference[0].id.match(/K\d{4,}/g)};b.prototype.saveCache=function(a){var b=[],c=0;for(a=a.children;c<a.length;c++){var d=a[c],e=d.name.toLowerCase().replace(/\s+/ig,"_");b.push(e);localStorage.setItem(e,JSON.stringify(d))}localStorage.setItem(this.assemblyKey,
JSON.stringify(b))};b.prototype.loadCache=function(){var a=localStorage.getItem(this.assemblyKey),b={name:"ko00001",children:[]},c=0;for(a=JSON.parse(a);c<a.length;c++){var d=a[c];TypeScript.logging.log("load cache: "+d+"...");d=localStorage.getItem(d);b.children.push(JSON.parse(d))}this.loadUITree(b)};return b}(Bootstrap);e.PathwayExplorer=d})(apps||(apps={}));var PathwayNavigator;
(function(e){function d(b){var a=KEGG.brite.parseIDEntry(b.name),e=a.commonName||"KEGG";return isNullOrEmpty(b.children)?{id:a.id+"_"+ ++c,text:b.name,children:null,icon:"data:image/png;base64,"}:
{id:a.id+"_"+ ++c,text:e,children:$from(b.children).Select(d).ToArray(),icon:"data:image/png;base64,"}}
var c=1;e.parseJsTree=d})(PathwayNavigator||(PathwayNavigator={}));
var graph=function(){function e(){this.last_index=0}e.prototype.objectify=function(){for(var d=this,c=d.links,b=[],a=function(a,e){var f=c[a];b.push(function(){var a;var b=d.nodes;var c=[];var e=0;for(a=b.length;e<a;e++){var g=b[e];f.source===g.id?f.source=g:f.target===g.id?f.target=g:c.push(void 0)}return c}())},e=0,g=c.length;e<g;e++)a(e,g);return b};e.prototype.remove=function(d){if(0<=__indexOf.call(this.nodes,d))return this.nodes=this.nodes.filter(function(c){return c!==d}),this.links=this.links.filter(function(c){return c.source.id!==
d.id&&c.target.id!==d.id});if(0<=__indexOf.call(this.links,d))return this.links=this.links.filter(function(c){return c!==d})};e.prototype.add_node=function(d){d={dunnartid:(this.last_index++).toString(),x:0,y:0,type:d};this.nodes.push(d);return d};e.prototype.add_link=function(d,c){if(d===c)return null;for(var b=this.links,a=0,e=b.length;a<e;a++){var g=b[a];if(g.source===d&&g.target===c)return null}d={source:d,target:c};this.links.push(d);return d};return e}(),dataAdapter;
(function(e){var d=function(){function c(b){void 0===b&&(b="#embeddedsvg");this.nodeLookup={};this.sbsvg=d3.select($ts(b)).select("svg");this.graph={nodes:[],links:[],constraints:[],groups:[]}}c.prototype.getGraph=function(){var b=this;this.loadNodes(this.sbsvg);for(var a=0,c=this.sbsvg.selectAll(".connector")[0];a<c.length;a++){var d=c[a],e=d.getAttribute("dunnart:srcid");d=d.getAttribute("dunnart:dstid");e in this.nodeLookup?d in this.nodeLookup?this.graph.links.push({source:this.nodeLookup[e].index,
target:this.nodeLookup[d].index}):console.warn("missing node of "+d):console.warn("missing node of "+e)}a=0;for(c=this.sbsvg.selectAll(".cluster")[0];a<c.length;a++)e=c[a],this.graph.groups.push({leaves:e.getAttribute("dunnart:contains").split(" ").map(function(a){return b.nodeLookup[a].index}),style:e.getAttribute("style"),padding:10});a={};c=0;for(e=this.sbsvg.selectAll("[relType=alignment]")[0];c<e.length;c++){var h=e[c];d=h.getAttribute("constraintid");var k=this.nodeLookup[h.getAttribute("objoneid")].index;
h=parseInt(h.getAttribute("alignmentpos"));k={node:k,offset:0};d in a?a[d].offsets.push(k):this.graph.constraints.push(a[d]={type:"alignment",offsets:[k],axis:1==h?"y":"x"})}return this.graph};c.prototype.loadNodes=function(b){var a=0,c=0;for(b=b.selectAll("rect.shape")[0];c<b.length;c++){var d=b[c];this.graph.nodes.push(this.nodeLookup[d.id]={label:d.getAttribute("dunnart:label"),dunnartid:d.id,index:a++,width:60,height:40,x:parseFloat(d.getAttribute("x")),y:parseFloat(d.getAttribute("y")),rx:"rx"in
d.attributes?parseFloat(d.getAttribute("rx")):5,ry:"ry"in d.attributes?parseFloat(d.getAttribute("ry")):5})}};return c}();e.parseDunnart=d})(dataAdapter||(dataAdapter={}));
