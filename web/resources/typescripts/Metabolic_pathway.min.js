...
this.height]);this.outer=d3.select($ts("#canvas")).append("svg").attr("width",this.width).attr("height",this.height).attr("pointer-events","all");this.outer.append("rect").attr("class","background").attr("width","100%").attr("height","100%").call(d3.behavior.zoom().on("zoom",function(){return c.redraw()}));this.vis=this.outer.append("g").attr("transform","translate(80,80) scale(0.7)");if("#"==a.charAt(0))this.d3cola.on("tick",this.loadGraph((new dataAdapter.parseDunnart(a)).getGraph()).tick());else $ts.getText(a,
function(a){return c.d3cola.on("tick",c.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};d.prototype.loadGraph=function(c){var a=this,g=this.vis.append("g"),f=this.vis.append("g"),e=this.vis.append("g");Strings.Empty(c.class)||"GraphLinksModel"!=c.class||(c=b.translation.translateToColaGraph(c));console.log(JSON.stringify(c));console.log("view nodes details:");console.table(c.nodes);this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();
this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",5).attr("markerWidth",3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");g=g.selectAll(".group").data(c.groups).enter().append("rect").attr("rx",8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});e=e.selectAll(".link").data(c.links).enter().append("line").attr("class",
"link");var h=f.selectAll(".node").data(c.nodes).enter().append("rect").attr("class","node").attr("width",function(c){return c.width+2*a.pad+2*a.margin}).attr("height",function(c){return c.height+2*a.pad+2*a.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);c=f.selectAll(".label").data(c.nodes).enter().append("text").attr("class","label").call(this.d3cola.drag);c.each(d.insertLinebreaks);h.append("title").text(function(a){return a.label});return new b.Cola_graph(h,
e,g,c,this)};d.insertLinebreaks=function(c){var a=d3.select(this);c=c.label.split(" ");a.text("");for(var b=0;b<c.length;b++){var f=c[b];f=a.append("tspan").text(f);f.attr("x",0).attr("dy","15").attr("font-size","12")}};return d}();b.Metabolic_pathway=d})(apps||(apps={}));
