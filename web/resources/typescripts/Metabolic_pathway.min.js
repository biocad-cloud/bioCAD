...
(function(b){function d(c){return c?c.toString().replace(".","_").replace("-","_").replace("+","_").replace("*","_").replace("~","_"):null}var f=/[-]?\d+/ig;b.ModelPatch=function(c){for(var a=0,b=c.nodeDataArray;a<b.length;a++){var e=b[a];e.isGroup||(Strings.IsPattern(e.key.toString(),f)&&(e.key="T"+e.key),e.key=d(e.key))}a=0;for(b=c.linkDataArray;a<b.length;a++)if(e=b[a],Strings.IsPattern(e.from.toString(),f)&&(e.from="T"+e.from),Strings.IsPattern(e.to.toString(),f)&&(e.to="T"+e.to),e.from=d(e.from),
e.to=d(e.to),!isNullOrEmpty(e.labelKeys))for(var k=0;k<e.labelKeys.length;k++)Strings.IsPattern(e.labelKeys[k].toString(),f)&&(e.labelKeys[k]="T"+e.labelKeys[k]),e.labelKeys[k]=d(e.labelKeys[k]);return c}})(apps||(apps={}));
...
e);d=[];for(var f=0,l=$from(e).GroupBy(function(a){return a.text.split(/\s+/ig)[0]}).Select(function(a){return a.First}).ToArray();f<l.length;f++){var h=l[f];d.push(new Application.Suggestion.term(h.id,h.text))}var m=Application.Suggestion.render.makeSuggestions(d,b.listDiv,function(b){return a.clickOnTerm(b)},13,!0,"");$ts(b.inputDiv).onkeyup=function(){var a=$ts.value(b.inputDiv);Strings.Empty(a,!0)?$ts(b.listDiv).hide():($ts(b.listDiv).show(),m(a))};TypeScript.logging.log(e.length+" kegg components has been loaded!",
TypeScript.ConsoleColors.Magenta);TypeScript.logging.log(" ~done!",TypeScript.ConsoleColors.Magenta)};c.prototype.clickOnTerm=function(a){$ts(b.listDiv).hide();console.log(a)};c.createSet=function(a,b){var c=0;for(a=a.children;c<a.length;c++){var d=a[c];isNullOrUndefined(d.children)?b.push(d):this.createSet(d,b)}};return c}(Bootstrap);b.KEGGNetwork=d})(apps||(apps={}));
