...
function(c){var a=this,g=this.vis.append("g"),f=this.vis.append("g"),e=this.vis.append("g");Strings.Empty(c.class)||"GraphLinksModel"!=c.class||(c=b.translation.translateToColaGraph(c));console.log(JSON.stringify(c));console.log("view nodes details:");console.table(c.nodes);this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",5).attr("markerWidth",
3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");g=g.selectAll(".group").data(c.groups).enter().append("rect").attr("rx",8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});e=e.selectAll(".link").data(c.links).enter().append("line").attr("class","link");var h=f.selectAll(".node").data(c.nodes).enter().append("rect").attr("class","node").attr("width",function(c){return c.width+
