...
(function(d){function b(b){return b.toString().replace(".","_").replace("-","_").replace("+","_").replace("*","_").replace("~","_")}var f=/[-]?\d+/ig;d.ModelPatch=function(c){for(var a=0,d=c.nodeDataArray;a<d.length;a++){var e=d[a];Strings.IsPattern(e.key.toString(),f)&&(e.key="T"+e.key);Strings.IsPattern(e.group.toString(),f)&&(e.group="T"+e.group);e.key=b(e.key);e.group=b(e.group)}a=0;for(d=c.linkDataArray;a<d.length;a++)if(e=d[a],Strings.IsPattern(e.from.toString(),f)&&(e.from="T"+e.from),Strings.IsPattern(e.to.toString(),
f)&&(e.to="T"+e.to),e.from=b(e.from),e.to=b(e.to),!isNullOrEmpty(e.labelKeys))for(var h=0;h<e.labelKeys.length;h++)Strings.IsPattern(e.labelKeys[h].toString(),f)&&(e.labelKeys[h]="T"+e.labelKeys[h]),e.labelKeys[h]=b(e.labelKeys[h]);return c}})(apps||(apps={}));
...
target:this.nodeLookup[e].index}):console.warn("missing node of "+e):console.warn("missing node of "+f)}a=0;for(d=this.sbsvg.selectAll(".cluster")[0];a<d.length;a++)f=d[a],this.graph.groups.push({leaves:f.getAttribute("dunnart:contains").split(" ").map(function(a){return b.nodeLookup[a].index}),style:f.getAttribute("style"),padding:10});a={};d=0;for(f=this.sbsvg.selectAll("[relType=alignment]")[0];d<f.length;d++){var k=f[d];e=k.getAttribute("constraintid");var l=this.nodeLookup[k.getAttribute("objoneid")].index;
