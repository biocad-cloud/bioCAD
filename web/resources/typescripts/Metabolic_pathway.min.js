...
function(a){return c.d3cola.on("tick",c.loadGraph(JSON.parse(a)).tick())});TypeScript.logging.log("intialization job done!",TypeScript.ConsoleColors.DarkBlue);return this};b.prototype.loadGraph=function(c){var a=this,g=this.vis.append("g"),e=this.vis.append("g"),h=this.vis.append("g");console.log(JSON.stringify(c));this.d3cola.nodes(c.nodes).links(c.links).groups(c.groups).constraints(c.constraints).start();this.outer.append("svg:defs").append("svg:marker").attr("id","end-arrow").attr("viewBox","0 -5 10 10").attr("refX",
5).attr("markerWidth",3).attr("markerHeight",3).attr("orient","auto").append("svg:path").attr("d","M0,-5L10,0L0,5L2,0").attr("stroke-width","0px").attr("fill","#000");g=g.selectAll(".group").data(c.groups).enter().append("rect").attr("rx",8).attr("ry",8).attr("class","group").attr("style",function(a){return a.style});h=h.selectAll(".link").data(c.links).enter().append("line").attr("class","link");var f=e.selectAll(".node").data(c.nodes).enter().append("rect").attr("class","node").attr("width",function(c){return c.width+
2*a.pad+2*a.margin}).attr("height",function(c){return c.height+2*a.pad+2*a.margin}).attr("rx",function(a){return a.rx}).attr("ry",function(a){return a.rx}).call(this.d3cola.drag);c=e.selectAll(".label").data(c.nodes).enter().append("text").attr("class","label").call(this.d3cola.drag);c.each(b.insertLinebreaks);f.append("title").text(function(a){return a.label});return new d.Cola_graph(f,h,g,c,this)};b.insertLinebreaks=function(c){var a=d3.select(this);c=c.label.split(" ");a.text("");for(var b=0;b<
c.length;b++){var e=c[b];e=a.append("tspan").text(e);e.attr("x",0).attr("dy","15").attr("font-size","12")}};return b}();d.Metabolic_pathway=b})(apps||(apps={}));
...
c)}}};c.prototype.init=function(){var a=go.GraphObject.make,c=this.SD,e=this.myDiagram=a(go.Diagram,"myDiagram",this.config());e.toolManager.mouseMoveTools.insertAt(0,new NodeLabelDraggingTool);e.addDiagramListener("Modified",function(a){a=document.getElementById("SaveButton");var c=document.title.indexOf("*");a&&(a.disabled=!e.isModified);e.isModified?0>c&&(document.title+="*"):0<=c&&(document.title=document.title.substr(0,c))});e.addDiagramListener("LinkDrawn",function(a){var b=a.subject;"flow"===
...
(function(d){function b(c){return c?c.toString().replace(".","_").replace("-","_").replace("+","_").replace("*","_").replace("~","_"):null}var f=/[-]?\d+/ig;d.ModelPatch=function(c){for(var a=0,d=c.nodeDataArray;a<d.length;a++){var e=d[a];Strings.IsPattern(e.key.toString(),f)&&(e.key="T"+e.key);e.group&&Strings.IsPattern(e.group.toString(),f)&&(e.group="T"+e.group);e.key=b(e.key);e.group=b(e.group)}a=0;for(d=c.linkDataArray;a<d.length;a++)if(e=d[a],Strings.IsPattern(e.from.toString(),f)&&(e.from=
"T"+e.from),Strings.IsPattern(e.to.toString(),f)&&(e.to="T"+e.to),e.from=b(e.from),e.to=b(e.to),!isNullOrEmpty(e.labelKeys))for(var h=0;h<e.labelKeys.length;h++)Strings.IsPattern(e.labelKeys[h].toString(),f)&&(e.labelKeys[h]="T"+e.labelKeys[h]),e.labelKeys[h]=b(e.labelKeys[h]);return c}})(apps||(apps={}));
