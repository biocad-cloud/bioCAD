...
(function(a){(function(a){a.show=function(b,f,c){var d=$ts(b),l=$from(c).ToDictionary(function(a){return a.classID.toString()},function(a){return a});f=$from(f).Select(function(c){return new a.bioCADFile(c,l)}).Select(function(c){var d=a.bioMimeTypes.classToFontAwsome(c.mime.class);return new a.FileHandle(c,d)});d.classList.contains(containerClassName)||d.classList.add(containerClassName);c=0;for(var m=f.ToArray();c<m.length;c++){var g=m[c];d.appendChild(g.getNode());g.handleEvents()}return{container:d,
files:f.ToArray(),divId:b}}})(a.Explorer||(a.Explorer={}))})(Application||(Application={}));(function(a){(function(a){var b=function(){function a(a,d){this.id=a.id;this.fileName=a.name;this.size=a.size;this.mime=d.Item(a.content_type)}a.prototype.toString=function(){return this.fileName};return a}();a.bioCADFile=b})(a.Explorer||(a.Explorer={}))})(Application||(Application={}));
...
Strings.Lanudry(parseFloat(this.file.size.toString()))+")</samp>\n                    </div>\n                </div>"};a.prototype.actionButtons=function(){return'<div class="file-actions">\n                    <div class="file-footer-buttons">\n                        <button type="button" \n                                class="kv-file-remove btn btn-sm btn-kv btn-default btn-outline-secondary" \n                                title="Delete file" \n                                data-url="/site/file-delete" \n                                data-key="'+
this.fileId+'">\n\n                            <i class="glyphicon glyphicon-trash">\n                            </i>\n                        </button>\n                        <button id="view-'+this.fileId+'" type="button" class="kv-file-zoom btn btn-sm btn-kv btn-default btn-outline-secondary" title="View Details">\n                            <i class="glyphicon glyphicon-zoom-in"></i>\n                        </button>\n                    </div>\n                </div>'};a.prototype.handleEvents=
function(){var a=this;$ts("#view-"+this.fileId).onclick=function(){a.viewer_click()}};a.prototype.viewer_click=function(){$ts("#diag-title").clear().innerText="View Model";$ts("#diag-body").clear().display($ts("<iframe>",{src:"@view:model?guid="+this.fileId+"&iframe=true&debugger=false",width:"1500px",height:"650px",frameborder:"no",border:"0",marginwidth:"0",marginheight:"0",scrolling:"no",allowtransparency:"yes"}));$ts("#viewer-modal").style.zIndex="1050000";$("#viewer-modal").modal("show")};a.prototype.getNode=
function(){var a=this.mimeIcon[0],d=this.mimeIcon[1];a=$ts("<div>",{class:"kv-file-content"}).display($ts("<div>",{class:["kv-preview-data","file-preview-other-frame"],style:"width:auto;height:auto;max-width:100%;max-height:100%;"}).display('\n                <div class="file-preview-other">\n                    <span class="file-other-icon">\n                        <a href="/biostack/pathway_design/view?guid='+this.fileId+'">\n                            <center>\n                                <div style="max-width: 128px; height: 50px; color: '+
d+';">\n                                    '+a+"\n                                </div>\n                            </center>\n                        </a>\n                    </span>\n                </div>\n            "));d=$ts("<div>",{class:"file-thumbnail-footer"}).appendElement(this.footer()).appendElement(this.actionButtons()).appendElement($ts("<div>",{class:"clearfix"}));return $ts("<div>",{class:["file-preview-frame","krajee-default","file-preview-initial","file-sortable","kv-preview-thumb"],
id:"r-"+this.fileId,"data-fileindex":this.fileId,"data-template":"image",title:this.file.fileName}).appendElement(a).appendElement(d)};a.classNames=["file-preview-frame","krajee-default","file-preview-initial","file-sortable","kv-preview-thumb"];return a}();a.FileHandle=b})(a.Explorer||(a.Explorer={}))})(Application||(Application={}));
...
data:Base64.encode(this.csvText)};DOM.download("PLAS_output.csv",a,!1)};c.prototype.initPathwaySelector=function(b){var c=this,d=$ts("#pathway_list"),e=$ts("<optgroup>",{label:"Pathways"}),h=$ts("<optgroup>",{label:"Metabolites"}),f=this.pathways,l=this,v=[];e.appendChild($ts("<option>",{value:"*"}).display("*"));for(var n=0,r=b.nodeDataArray;n<r.length;n++){var k=r[n];if(k.isGroup){var x={pathway:k.label,keys:[]},p=$ts("<option>",{value:k.key}),w=new Application.Suggestion.term(k.key,k.text);p.innerText=
...
Strings.Empty(a)||l.updateChart(a.toString())};var y=Application.Suggestion.render.makeSuggestions(v,a.listDiv,function(a){return c.clickOnTerm(a)},5,!0,"");$ts(a.inputDiv).onkeyup=function(){var b=$ts.value(a.inputDiv);Strings.Empty(b,!0)?$ts(a.listDiv).hide():($ts(a.listDiv).show(),y(b))}};c.prototype.clickOnTerm=function(b){$ts.value("#pathway_list",b.id.toString());$ts(a.listDiv).hide();this.updateChart(b.id)};return c}(Bootstrap);a.Report=b})(a.Platform||(a.Platform={}))})(a.WebApp||(a.WebApp=
{}))})(bioCAD||(bioCAD={}));
...
a.sha1+'" target="__blank">\n                                <h4 class="mb-0">'+a.title+"</h4>\n                            </a>                            \n                        </div>")),g=$ts("<td>").appendElement(a.create_time),h=$ts("<td>").appendElement(m[b(a.status)]),l=$ts("<td>").appendElement(a.finish_time||"n/a");a=$ts("<td>").appendElement(e[b(a.status)]);c=$ts("<td>",{class:"text-right"}).appendElement(d.replace("{$model_url}",c));return $ts("<tr>").appendElement(f).appendElement(g).appendElement(h).appendElement(l).appendElement(a).appendElement(c)};
