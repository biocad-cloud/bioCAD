...
console.log(c);d&&b.setOption(d);b.on("legendselectchanged",function(a){console.log(a)})};b.prototype.init=function(){var a=this,b=document.getElementById("main"),c=echarts.init(b);$ts.getText("@data:PLAS",function(b){return a.makeChart($ts.csv(b,!1),c)});$ts.getText("@url:graph",function(b){return a.initPathwaySelector(JSON.parse(b))})};b.prototype.initPathwaySelector=function(a){for(var b=$ts("#pathway_list"),c=this.pathways,d=0,g=a.nodeDataArray;d<g.length;d++){var f=g[d];if(f.isGroup){var n={pathway:f.label,
keys:[]},e=$ts("<option>",{value:f.key});e.innerText=f.text;c.Add(f.key.toString(),n);b.add(e)}}d=0;for(a=a.nodeDataArray;d<a.length;d++)if(f=a[d],isNullOrUndefined(f.isGroup)||!f.isGroup)Strings.Empty(f.group,!0)||c.Item(f.group.toString()).keys.push(f.label);b.onchange=function(a,b){console.log(b)}};return b}(Bootstrap);a.Report=b})(a.Platform||(a.Platform={}))})(a.WebApp||(a.WebApp={}))})(bioCAD||(bioCAD={}));
...
a.sha1+'" target="__blank">\n                                <h4 class="mb-0">'+a.title+"</h4>\n                            </a>                            \n                        </div>")),e=$ts("<td>").appendElement(a.create_time),g=$ts("<td>").appendElement(m[b(a.status)]),h=$ts("<td>").appendElement(a.finish_time||"n/a");a=$ts("<td>").appendElement(l[b(a.status)]);c=$ts("<td>",{class:"text-right"}).appendElement(k.replace("{$model_url}",c));return $ts("<tr>").appendElement(d).appendElement(e).appendElement(g).appendElement(h).appendElement(a).appendElement(c)};
