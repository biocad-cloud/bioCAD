...
(function(a){(function(a){var b=function(){function b(a,d){this.divId=a.id;this.files=d;this.container=a}b.show=function(c,d,k){void 0===k&&(k=[]);c=document.getElementById(c);var f=$from(k).ToDictionary(function(a){return a.key},function(a){return a.value});d=$from(d).Select(function(c){var d=f.Item(c.mime.contentType);d=a.bioMimeTypes.classToFontAwsome(d);return new a.FileHandle(c,d)});c.classList.contains(containerClassName)||c.classList.add(containerClassName);c.innerHTML=d.Select(function(a){return a.toString()}).JoinBy("\n\n");
return new b(c,d.ToArray())};b.getFaMaps=function(a){a=$from(LoadJson(a)).Select(function(a){return new MapTuple(a.content_type,a.classId)}).ToArray();console.log(a);return a};b.getFiles=function(c,d){var b=$from(LoadJson(d)).ToDictionary(function(a){return a.id},function(a){return new BioCAD.MIME.mimeType(a)});c=$from(LoadJson(c)).Select(function(c){return new a.bioCADFile(c,b)}).ToArray();console.log(c);return c};return b}();a.Explorer=b})(a.Explorer||(a.Explorer={}))})(Application||(Application=
{}));(function(a){(function(a){var b=function(){function a(a,b){this.id=a.id;this.fileName=a.name;this.size=a.size;this.mime=b.Item(a.content_type)}a.prototype.toString=function(){return this.fileName};return a}();a.bioCADFile=b})(a.Explorer||(a.Explorer={}))})(Application||(Application={}));
...
l=f.Where(function(b){return b.score!=a.NA}).Take(d).Select(function(a){return a.term}).ToArray();return l.length==d?l:b.makeAdditionalSuggestion(f,l,k,c,d)};b.makeAdditionalSuggestion=function(a,b,k,e,f){var c=e.toLowerCase();a=a.Skip(b.length).Select(function(a){a=a.term;var b=k?Levenshtein.ComputeDistance(a.term.toLowerCase(),c):Levenshtein.ComputeDistance(a.term,e);return{term:a,score:b}}).OrderBy(function(a){return a.score}).Take(f-b.length).Select(function(a){return a.term}).ToArray();return b.concat(a)};
...
(function(a){(function(a){(function(a){var b=function(a){function b(){return null!==a&&a.apply(this,arguments)||this}__extends(b,a);Object.defineProperty(b.prototype,"appName",{get:function(){return"FileExplorer"},enumerable:!0,configurable:!0});b.prototype.init=function(){throw Error("Method not implemented.");};return b}(Bootstrap);a.FileManager=b})(a.Platform||(a.Platform={}))})(a.WebApp||(a.WebApp={}))})(bioCAD||(bioCAD={}));
...
data:Base64.encode(this.csvText)};DOM.download("PLAS_output.csv",a,!1)};c.prototype.initPathwaySelector=function(b){var c=this,d=$ts("#pathway_list"),e=$ts("<optgroup>",{label:"Pathways"}),h=$ts("<optgroup>",{label:"Metabolites"}),f=this.pathways,k=this,v=[];e.appendChild($ts("<option>",{value:"*"}).display("*"));for(var n=0,r=b.nodeDataArray;n<r.length;n++){var g=r[n];if(g.isGroup){var x={pathway:g.label,keys:[]},p=$ts("<option>",{value:g.key}),w=new Application.Suggestion.term(g.key,g.text);p.innerText=
g.text;v.push(w);f.Add(g.key.toString(),x);e.appendChild(p)}}n=0;for(b=b.nodeDataArray;n<b.length;n++)g=b[n],!isNullOrUndefined(g.isGroup)&&g.isGroup||Strings.Empty(g.group,!0)||(r=f.Item(g.group.toString()),p=$ts("<option>",{value:g.label}),w=new Application.Suggestion.term(g.label,g.label),p.innerText=g.label,r.keys.push(g.label),"valve"==g.category||Strings.Empty(g.label,!0)||(v.push(w),h.appendChild(p)));d.add(e);d.add(h);d.onchange=function(){var a=$ts.select.getOption("#pathway_list");isNullOrUndefined(a)||
Strings.Empty(a)||k.updateChart(a.toString())};var y=Application.Suggestion.render.makeSuggestions(v,a.listDiv,function(a){return c.clickOnTerm(a)},5,!0,"");$ts(a.inputDiv).onkeyup=function(){var b=$ts.value(a.inputDiv);Strings.Empty(b,!0)?$ts(a.listDiv).hide():($ts(a.listDiv).show(),y(b))}};c.prototype.clickOnTerm=function(b){$ts.value("#pathway_list",b.id.toString());$ts(a.listDiv).hide();this.updateChart(b.id)};return c}(Bootstrap);a.Report=b})(a.Platform||(a.Platform={}))})(a.WebApp||(a.WebApp=
{}))})(bioCAD||(bioCAD={}));
...
a.sha1+'" target="__blank">\n                                <h4 class="mb-0">'+a.title+"</h4>\n                            </a>                            \n                        </div>")),h=$ts("<td>").appendElement(a.create_time),k=$ts("<td>").appendElement(t[b(a.status)]),l=$ts("<td>").appendElement(a.finish_time||"n/a");a=$ts("<td>").appendElement(e[b(a.status)]);c=$ts("<td>",{class:"text-right"}).appendElement(d.replace("{$model_url}",c));return $ts("<tr>").appendElement(f).appendElement(h).appendElement(k).appendElement(l).appendElement(a).appendElement(c)};
