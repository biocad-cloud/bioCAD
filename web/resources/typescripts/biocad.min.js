...
function(a){var b=TypeScript.Data.quantile(a.Select(function(a){return a.ymax}).ToArray(),.65),c=this.myChart;b={animation:!1,tooltip:{trigger:"none",axisPointer:{type:"cross"}},legend:{},grid:{top:40,left:50,right:40,bottom:50},xAxis:{name:"Time(ticks)",minorTick:{show:!0},minorSplitLine:{show:!0},nameLocation:"middle"},yAxis:{name:"Activity",min:0,max:b,minorTick:{show:!0},minorSplitLine:{show:!0}},series:a.ToArray()};console.log("lines:");console.log(a);b&&c.setOption(b)};b.prototype.init=function(){var a=
this,b=document.getElementById("main"),c=echarts.init(b);$ts.getText("@data:PLAS",function(b){return a.makeChart($ts.csv(b,!1),c)});$ts.getText("@url:graph",function(b){return a.initPathwaySelector(JSON.parse(b))})};b.prototype.initPathwaySelector=function(a){var b=$ts("#pathway_list"),c=$ts("<optgroup>",{label:"Pathways"}),d=$ts("<optgroup>",{label:"Metabolites"}),k=this.pathways,r=this;c.appendChild($ts("<option>",{value:"*"}).display("*"));for(var g=0,l=a.nodeDataArray;g<l.length;g++){var e=l[g];
if(e.isGroup){var f={pathway:e.label,keys:[]},h=$ts("<option>",{value:e.key});h.innerText=e.text;k.Add(e.key.toString(),f);c.appendChild(h)}}g=0;for(a=a.nodeDataArray;g<a.length;g++)e=a[g],!isNullOrUndefined(e.isGroup)&&e.isGroup||Strings.Empty(e.group,!0)||(l=k.Item(e.group.toString()),h=$ts("<option>",{value:e.key}),h.innerText=e.label,l.keys.push(e.label),"valve"==e.category||Strings.Empty(e.label,!0)||d.appendChild(h));b.add(c);b.add(d);b.onchange=function(){r.updateChart($ts.select.getOption("#pathway_list"))}};
...
a.sha1+'" target="__blank">\n                                <h4 class="mb-0">'+a.title+"</h4>\n                            </a>                            \n                        </div>")),f=$ts("<td>").appendElement(a.create_time),e=$ts("<td>").appendElement(p[b(a.status)]),k=$ts("<td>").appendElement(a.finish_time||"n/a");a=$ts("<td>").appendElement(n[b(a.status)]);c=$ts("<td>",{class:"text-right"}).appendElement(m.replace("{$model_url}",c));return $ts("<tr>").appendElement(d).appendElement(f).appendElement(e).appendElement(k).appendElement(a).appendElement(c)};
